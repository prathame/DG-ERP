---
sidebar_label: "AnalyticsView.tsx"
title: "File src/features/analytics/AnalyticsView.tsx"
description: "Deep walkthrough of src/features/analytics/AnalyticsView.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/features/analytics/AnalyticsView.tsx`
...
`src/features/analytics/AnalyticsView.tsx` is part of Dhandho (DG-ERP). Approximate size: **237 lines**.
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/features/analytics/AnalyticsView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `relativeTime` before deleting. |
...
```ts
AnalyticsView({ setActiveTab }: { setActiveTab: (tab: Tab)
```
...
| Purpose | Symbol in `src/features/analytics/AnalyticsView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `AnalyticsView` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "AnalyticsView" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **237**. Large view/route files are refactor candidates.
...
1. Open `src/features/analytics/AnalyticsView.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__features__analytics__analyticsview`*
