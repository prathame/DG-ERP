---
sidebar_label: "PlanManagementView.tsx"
title: "File src/features/super-admin/PlanManagementView.tsx"
description: "Deep walkthrough of src/features/super-admin/PlanManagementView.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/features/super-admin/PlanManagementView.tsx`
...
`src/features/super-admin/PlanManagementView.tsx` is part of Dhandho (DG-ERP). Approximate size: **384 lines**.
...
**Exported names:** `PlanManagementView`
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/features/super-admin/PlanManagementView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `PlanManagementView` before deleting. |
...
```ts
PlanModal({ plan, onClose, onSaved }: {
  plan: Plan | null;
  onClose: ()
```
...
| Purpose | Symbol in `src/features/super-admin/PlanManagementView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `PlanModal` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `src/features/super-admin/PlanManagementView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `fetchPlans` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `src/features/super-admin/PlanManagementView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `confirmDelete` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "PlanManagementView" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **384**. Large view/route files are refactor candidates.
...
1. Open `src/features/super-admin/PlanManagementView.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__features__super-admin__planmanagementview`*
