---
sidebar_label: "Skeleton.tsx"
title: "File src/components/ui/Skeleton.tsx"
description: "Deep walkthrough of src/components/ui/Skeleton.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/components/ui/Skeleton.tsx`
...
`src/components/ui/Skeleton.tsx` is part of Dhandho (DG-ERP). Approximate size: **53 lines**.
...
**Exported names:** `Skeleton`, `DashboardSkeleton`, `TableSkeleton`
...
```ts
Skeleton({ className }: { key?: Key; className?: string })
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/components/ui/Skeleton.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `Skeleton` before deleting. |
...
| Purpose | Symbol in `src/components/ui/Skeleton.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `DashboardSkeleton` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
```ts
TableSkeleton({ rows = 5, cols = 4 }: { rows?: number; cols?: number })
```
...
| Purpose | Symbol in `src/components/ui/Skeleton.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `TableSkeleton` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "Skeleton" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `src/components/ui/Skeleton.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__components__ui__skeleton`*
