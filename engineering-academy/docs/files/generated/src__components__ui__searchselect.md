---
sidebar_label: "SearchSelect.tsx"
title: "File src/components/ui/SearchSelect.tsx"
description: "Deep walkthrough of src/components/ui/SearchSelect.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/components/ui/SearchSelect.tsx`
...
`src/components/ui/SearchSelect.tsx` is part of Dhandho (DG-ERP). Approximate size: **130 lines**.
...
```ts
SearchSelect({ options, value, onChange, placeholder = 'Search...', className }: SearchSelectProps)
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/components/ui/SearchSelect.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `SearchSelect` before deleting. |
...
| Purpose | Symbol in `src/components/ui/SearchSelect.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `updatePos` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "SearchSelect" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `src/components/ui/SearchSelect.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__components__ui__searchselect`*
