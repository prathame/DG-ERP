---
sidebar_label: "Pagination.tsx"
title: "File src/components/ui/Pagination.tsx"
description: "Deep walkthrough of src/components/ui/Pagination.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/components/ui/Pagination.tsx`
...
`src/components/ui/Pagination.tsx` is part of Dhandho (DG-ERP). Approximate size: **22 lines**.
...
**Exported names:** `PaginationControls`
...
```ts
PaginationControls({ page, totalPages, total, onPageChange }: { page: number; totalPages: number; total: number; onPageChange: (p: number)
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/components/ui/Pagination.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `PaginationControls` before deleting. |
...
rg -n "Pagination" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `src/components/ui/Pagination.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__components__ui__pagination`*
