---
sidebar_label: "DeskIllustration.tsx"
title: "File src/components/layout/DeskIllustration.tsx"
description: "Deep walkthrough of src/components/layout/DeskIllustration.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/components/layout/DeskIllustration.tsx`
...
`src/components/layout/DeskIllustration.tsx` is part of Dhandho (DG-ERP). Approximate size: **150 lines**.
...
**Exported names:** `DeskIllustration`
...
```ts
DeskIllustration({ dark }: { dark: boolean })
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/components/layout/DeskIllustration.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `DeskIllustration` before deleting. |
...
rg -n "DeskIllustration" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **150**. Large view/route files are refactor candidates.
...
1. Open `src/components/layout/DeskIllustration.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__components__layout__deskillustration`*
