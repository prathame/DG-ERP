---
sidebar_label: "hospitalitySeed.ts"
title: "File server/utils/hospitalitySeed.ts"
description: "Deep walkthrough of server/utils/hospitalitySeed.ts in DG-ERP / Dhandho"
---

# File walkthrough: `server/utils/hospitalitySeed.ts`
...
`server/utils/hospitalitySeed.ts` is part of Dhandho (DG-ERP). Approximate size: **126 lines**.
...
**Exported names:** `seedHospitalityCatalog`

**Classes:** _none_

## Functions (2 detected)

### Function: seedHospitalityCatalog

```ts
seedHospitalityCatalog(tenantId: string)
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `server/utils/hospitalitySeed.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `seedHospitalityCatalog` before deleting. |
...
| Purpose | Symbol in `server/utils/hospitalitySeed.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `link` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "hospitalitySeed" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **126**. Large view/route files are refactor candidates.
...
1. Open `server/utils/hospitalitySeed.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `server__utils__hospitalityseed`*
