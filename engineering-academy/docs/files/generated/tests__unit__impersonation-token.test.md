---
sidebar_label: "impersonation-token.test.ts"
title: "File tests/unit/impersonation-token.test.ts"
description: "Deep walkthrough of tests/unit/impersonation-token.test.ts in DG-ERP / Dhandho"
---

# File walkthrough: `tests/unit/impersonation-token.test.ts`
...
`tests/unit/impersonation-token.test.ts` is part of Dhandho (DG-ERP). Approximate size: **29 lines**.
...
rg -n "impersonation-token.test" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `tests/unit/impersonation-token.test.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `tests__unit__impersonation-token.test`*
