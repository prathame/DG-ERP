---
sidebar_label: "http-hospitality.test.ts"
title: "File tests/api/http-hospitality.test.ts"
description: "Deep walkthrough of tests/api/http-hospitality.test.ts in DG-ERP / Dhandho"
---

# File walkthrough: `tests/api/http-hospitality.test.ts`
...
`tests/api/http-hospitality.test.ts` is part of Dhandho (DG-ERP). Approximate size: **152 lines**.
...
```ts
token(tenantId: string, userId: string)
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `tests/api/http-hospitality.test.ts`. Open the source and read the body. |
...
```ts
seedTenant(id: string, businessType: string, userId: string)
```
...
| Purpose | Symbol in `tests/api/http-hospitality.test.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `seedTenant` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `tests/api/http-hospitality.test.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `free` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "http-hospitality.test" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **152**. Large view/route files are refactor candidates.
...
1. Open `tests/api/http-hospitality.test.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `tests__api__http-hospitality.test`*
