---
sidebar_label: "http-mobile.test.ts"
title: "File tests/api/http-mobile.test.ts"
description: "Deep walkthrough of tests/api/http-mobile.test.ts in DG-ERP / Dhandho"
---

# File walkthrough: `tests/api/http-mobile.test.ts`
...
`tests/api/http-mobile.test.ts` is part of Dhandho (DG-ERP). Approximate size: **197 lines**.
...
rg -n "http-mobile.test" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **197**. Large view/route files are refactor candidates.
...
1. Open `tests/api/http-mobile.test.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `tests__api__http-mobile.test`*
