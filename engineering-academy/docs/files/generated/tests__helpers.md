...
title: "File tests/helpers.ts"
description: "Deep walkthrough of tests/helpers.ts in DG-ERP / Dhandho"
---

# File walkthrough: `tests/helpers.ts`
...
`tests/helpers.ts` is part of Dhandho (DG-ERP). Approximate size: **74 lines**.
...
**Exported names:** `createTestToken`, `createSuperAdminToken`, `cleanupTestData`
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `tests/helpers.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `requireJwtSecret` before deleting. |
...
| Purpose | Symbol in `tests/helpers.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `createTestToken` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `tests/helpers.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `createSuperAdminToken` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
```ts
cleanupTestData(tenantId: string)
```
...
| Purpose | Symbol in `tests/helpers.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `cleanupTestData` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
1. Open `tests/helpers.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `tests__helpers`*
