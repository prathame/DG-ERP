---
sidebar_label: "PriceListView.tsx"
title: "File src/features/masters/PriceListView.tsx"
description: "Deep walkthrough of src/features/masters/PriceListView.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/features/masters/PriceListView.tsx`
...
| Purpose | Symbol in `src/features/masters/PriceListView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `generatePriceListText` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `src/features/masters/PriceListView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `generatePriceListHtml` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "PriceListView" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `src/features/masters/PriceListView.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__features__masters__pricelistview`*
