---
sidebar_label: "hsnRates.ts"
title: "File src/lib/hsnRates.ts"
description: "Deep walkthrough of src/lib/hsnRates.ts in DG-ERP / Dhandho"
---

# File walkthrough: `src/lib/hsnRates.ts`
...
`src/lib/hsnRates.ts` is part of Dhandho (DG-ERP). Approximate size: **117 lines**.
...
**Exported names:** `suggestHsnRate`

**Classes:** _none_

## Functions (1 detected)

### Function: suggestHsnRate

```ts
suggestHsnRate(code: string)
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/lib/hsnRates.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `suggestHsnRate` before deleting. |
...
rg -n "hsnRates" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **117**. Large view/route files are refactor candidates.
...
1. Open `src/lib/hsnRates.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__lib__hsnrates`*
