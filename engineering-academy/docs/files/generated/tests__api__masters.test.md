---
sidebar_label: "masters.test.ts"
title: "File tests/api/masters.test.ts"
description: "Deep walkthrough of tests/api/masters.test.ts in DG-ERP / Dhandho"
---

# File walkthrough: `tests/api/masters.test.ts`
...
`tests/api/masters.test.ts` is part of Dhandho (DG-ERP). Approximate size: **108 lines**.
...
rg -n "masters.test" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `tests/api/masters.test.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `tests__api__masters.test`*
