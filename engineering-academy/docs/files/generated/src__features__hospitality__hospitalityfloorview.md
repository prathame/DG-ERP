---
sidebar_label: "HospitalityFloorView.tsx"
title: "File src/features/hospitality/HospitalityFloorView.tsx"
description: "Deep walkthrough of src/features/hospitality/HospitalityFloorView.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/features/hospitality/HospitalityFloorView.tsx`
...
`src/features/hospitality/HospitalityFloorView.tsx` is part of Dhandho (DG-ERP). Approximate size: **186 lines**.
...
**Exported names:** `HospitalityFloorView`, `HospitalityWaiterView`

**Classes:** _none_

## Functions (2 detected)

### Function: HospitalityFloorView

```ts
HospitalityFloorView({ title = 'Floor' }: { title?: string })
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/features/hospitality/HospitalityFloorView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `HospitalityFloorView` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |

### Function: HospitalityWaiterView

```ts
HospitalityWaiterView()
```
...
| Purpose | Symbol in `src/features/hospitality/HospitalityFloorView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `HospitalityWaiterView` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "HospitalityFloorView" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `src/features/hospitality/HospitalityFloorView.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__features__hospitality__hospitalityfloorview`*
