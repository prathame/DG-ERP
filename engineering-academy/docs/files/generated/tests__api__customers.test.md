---
sidebar_label: "customers.test.ts"
title: "File tests/api/customers.test.ts"
description: "Deep walkthrough of tests/api/customers.test.ts in DG-ERP / Dhandho"
---

# File walkthrough: `tests/api/customers.test.ts`
...
`tests/api/customers.test.ts` is part of Dhandho (DG-ERP). Approximate size: **112 lines**.
...
rg -n "customers.test" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `tests/api/customers.test.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `tests__api__customers.test`*
