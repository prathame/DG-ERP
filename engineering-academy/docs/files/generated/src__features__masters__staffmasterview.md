---
sidebar_label: "StaffMasterView.tsx"
title: "File src/features/masters/StaffMasterView.tsx"
description: "Deep walkthrough of src/features/masters/StaffMasterView.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/features/masters/StaffMasterView.tsx`
...
`src/features/masters/StaffMasterView.tsx` is part of Dhandho (DG-ERP). Approximate size: **297 lines**.
...
**Exported names:** `StaffMasterView`

**Classes:** _none_

## Functions (9 detected)

### Function: StaffMasterView

```ts
StaffMasterView({ onBack, onRefresh }: { onBack: ()
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/features/masters/StaffMasterView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `StaffMasterView` before deleting. |
...
| Purpose | Symbol in `src/features/masters/StaffMasterView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `selectStaff` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `src/features/masters/StaffMasterView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `openAdd` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `src/features/masters/StaffMasterView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `handleSaveStaff` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "StaffMasterView" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **297**. Large view/route files are refactor candidates.
...
1. Open `src/features/masters/StaffMasterView.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__features__masters__staffmasterview`*
