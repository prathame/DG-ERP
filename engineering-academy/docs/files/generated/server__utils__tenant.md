---
sidebar_label: "tenant.ts"
title: "File server/utils/tenant.ts"
description: "Deep walkthrough of server/utils/tenant.ts in DG-ERP / Dhandho"
---

# File walkthrough: `server/utils/tenant.ts`
...
`server/utils/tenant.ts` is part of Dhandho (DG-ERP). Approximate size: **130 lines**.
...
**Exported names:** `provisionTenant`, `deleteTenant`, `getTenantStats`
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `server/utils/tenant.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `provisionTenant` before deleting. |
...
| Purpose | Symbol in `server/utils/tenant.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `deleteTenant` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `server/utils/tenant.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `getTenantStats` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "tenant" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `server/utils/tenant.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `server__utils__tenant`*
