---
sidebar_label: "http-error.test.ts"
title: "File tests/unit/http-error.test.ts"
description: "Deep walkthrough of tests/unit/http-error.test.ts in DG-ERP / Dhandho"
---

# File walkthrough: `tests/unit/http-error.test.ts`
...
`tests/unit/http-error.test.ts` is part of Dhandho (DG-ERP). Approximate size: **151 lines**.
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `tests/unit/http-error.test.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `mockRes` before deleting. |
...
```ts
mockReq(overrides: Partial<Request> & Record<string, unknown> = {})
```
...
| Purpose | Symbol in `tests/unit/http-error.test.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `mockReq` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "http-error.test" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **151**. Large view/route files are refactor candidates.
...
1. Open `tests/unit/http-error.test.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `tests__unit__http-error.test`*
