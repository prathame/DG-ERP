---
sidebar_label: "cache.ts"
title: "File src/platforms/mobile/offline/cache.ts"
description: "Deep walkthrough of src/platforms/mobile/offline/cache.ts in DG-ERP / Dhandho"
---

# File walkthrough: `src/platforms/mobile/offline/cache.ts`
...
`src/platforms/mobile/offline/cache.ts` is part of Dhandho (DG-ERP). Approximate size: **46 lines**.
...
**Exported names:** `cacheSet`, `cacheGet`, `cacheClear`, `cacheInvalidateForApiPath`
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/platforms/mobile/offline/cache.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `cacheClear` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |

### Function: cacheInvalidateForApiPath

```ts
cacheInvalidateForApiPath(path: string)
```
...
| Purpose | Symbol in `src/platforms/mobile/offline/cache.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `cacheInvalidateForApiPath` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "cache" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **46**. Large view/route files are refactor candidates.
...
1. Open `src/platforms/mobile/offline/cache.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__platforms__mobile__offline__cache`*
