---
sidebar_label: "AppShutterIntro.tsx"
title: "File src/components/layout/AppShutterIntro.tsx"
description: "Deep walkthrough of src/components/layout/AppShutterIntro.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/components/layout/AppShutterIntro.tsx`
...
`src/components/layout/AppShutterIntro.tsx` is part of Dhandho (DG-ERP). Approximate size: **209 lines**.
...
**Exported names:** `AppShutterIntro`
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/components/layout/AppShutterIntro.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `playAppShutterSound` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |

### Function: AppShutterIntro

```ts
AppShutterIntro({ companyName, onDone }: Props)
```
...
| Purpose | Symbol in `src/components/layout/AppShutterIntro.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `AppShutterIntro` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "AppShutterIntro" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **209**. Large view/route files are refactor candidates.
...
1. Open `src/components/layout/AppShutterIntro.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__components__layout__appshutterintro`*
