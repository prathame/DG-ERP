---
sidebar_label: "DateRangeFilter.tsx"
title: "File src/components/ui/DateRangeFilter.tsx"
description: "Deep walkthrough of src/components/ui/DateRangeFilter.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/components/ui/DateRangeFilter.tsx`
...
`src/components/ui/DateRangeFilter.tsx` is part of Dhandho (DG-ERP). Approximate size: **23 lines**.
...
**Exported names:** `DateRangeFilter`
...
```ts
DateRangeFilter({ value, onChange }: { value: { range: string; from: string; to: string }; onChange: (v: { range: string; from: string; to: string })
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/components/ui/DateRangeFilter.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `DateRangeFilter` before deleting. |
...
rg -n "DateRangeFilter" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `src/components/ui/DateRangeFilter.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__components__ui__daterangefilter`*
