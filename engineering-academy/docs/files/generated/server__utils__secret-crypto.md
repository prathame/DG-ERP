---
sidebar_label: "secret-crypto.ts"
title: "File server/utils/secret-crypto.ts"
description: "Deep walkthrough of server/utils/secret-crypto.ts in DG-ERP / Dhandho"
---

# File walkthrough: `server/utils/secret-crypto.ts`
...
`server/utils/secret-crypto.ts` is part of Dhandho (DG-ERP). Approximate size: **39 lines**.
...
**Exported names:** `encryptSecret`, `decryptSecret`
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `server/utils/secret-crypto.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `keyFromEnv` before deleting. |
...
| Purpose | Symbol in `server/utils/secret-crypto.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `encryptSecret` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `server/utils/secret-crypto.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `decryptSecret` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "secret-crypto" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `server/utils/secret-crypto.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `server__utils__secret-crypto`*
