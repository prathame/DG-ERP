---
sidebar_label: "MobileOnboarding.tsx"
title: "File src/platforms/mobile/online/MobileOnboarding.tsx"
description: "Deep walkthrough of src/platforms/mobile/online/MobileOnboarding.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/platforms/mobile/online/MobileOnboarding.tsx`
...
**Exported names:** `MobileOnboarding`
...
| What breaks if removed | Search the repo for `normalizeSlug` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |

### Function: looksLikeInvite

```ts
looksLikeInvite(raw: string)
```
...
| Purpose | Symbol in `src/platforms/mobile/online/MobileOnboarding.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `looksLikeInvite` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
```ts
MobileOnboarding({ initialSlug = '', onComplete }: Props)
```
...
| Purpose | Symbol in `src/platforms/mobile/online/MobileOnboarding.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `MobileOnboarding` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `src/platforms/mobile/online/MobileOnboarding.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `continueOnboard` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "MobileOnboarding" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `src/platforms/mobile/online/MobileOnboarding.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__platforms__mobile__online__mobileonboarding`*
