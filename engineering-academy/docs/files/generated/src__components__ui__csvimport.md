---
sidebar_label: "CsvImport.tsx"
title: "File src/components/ui/CsvImport.tsx"
description: "Deep walkthrough of src/components/ui/CsvImport.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/components/ui/CsvImport.tsx`
...
`src/components/ui/CsvImport.tsx` is part of Dhandho (DG-ERP). Approximate size: **250 lines**.
...
**Exported names:** `CsvImportProps`, `CsvImport`
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/components/ui/CsvImport.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `parseCsv` before deleting. |
...
```ts
CsvImport({ onImport, onClose, columns, templateName, itemLabel = 'items', existingNames }: CsvImportProps)
```
...
| Purpose | Symbol in `src/components/ui/CsvImport.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `CsvImport` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `src/components/ui/CsvImport.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `handleImport` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `src/components/ui/CsvImport.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `downloadTemplate` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "CsvImport" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **250**. Large view/route files are refactor candidates.
...
1. Open `src/components/ui/CsvImport.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__components__ui__csvimport`*
