---
sidebar_label: "PurchasesView.tsx"
title: "File src/features/purchases/PurchasesView.tsx"
description: "Deep walkthrough of src/features/purchases/PurchasesView.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/features/purchases/PurchasesView.tsx`
...
`src/features/purchases/PurchasesView.tsx` is part of Dhandho (DG-ERP). Approximate size: **1233 lines**.
...
```ts
PurchasesView({ accessLevel = 'full' }: { accessLevel?: 'hidden' | 'view' | 'print' | 'full' } = {})
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/features/purchases/PurchasesView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `PurchasesView` before deleting. |
...
| Purpose | Symbol in `src/features/purchases/PurchasesView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `handleCreateSupplier` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `src/features/purchases/PurchasesView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `handleCreatePurchase` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "PurchasesView" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **1233**. Large view/route files are refactor candidates.
...
1. Open `src/features/purchases/PurchasesView.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__features__purchases__purchasesview`*
