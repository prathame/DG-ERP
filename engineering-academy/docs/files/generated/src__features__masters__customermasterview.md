---
sidebar_label: "CustomerMasterView.tsx"
title: "File src/features/masters/CustomerMasterView.tsx"
description: "Deep walkthrough of src/features/masters/CustomerMasterView.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/features/masters/CustomerMasterView.tsx`
...
`src/features/masters/CustomerMasterView.tsx` is part of Dhandho (DG-ERP). Approximate size: **161 lines**.
...
**Exported names:** `CustomerMasterView`
...
```ts
CustomerMasterView({ onBack, onRefresh, user }: { onBack: ()
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/features/masters/CustomerMasterView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `CustomerMasterView` before deleting. |
...
| Purpose | Symbol in `src/features/masters/CustomerMasterView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `openPurchases` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "CustomerMasterView" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **161**. Large view/route files are refactor candidates.
...
1. Open `src/features/masters/CustomerMasterView.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__features__masters__customermasterview`*
