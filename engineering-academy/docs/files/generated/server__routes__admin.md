---
sidebar_label: "admin.ts"
title: "File server/routes/admin.ts"
description: "Deep walkthrough of server/routes/admin.ts in DG-ERP / Dhandho"
---

# File walkthrough: `server/routes/admin.ts`
...
```ts
isAdmin(role: string | undefined)
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `server/routes/admin.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `isAdmin` before deleting. |
...
| Purpose | Symbol in `server/routes/admin.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `vendorExists` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `server/routes/admin.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `target` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "admin" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `server/routes/admin.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `server__routes__admin`*
