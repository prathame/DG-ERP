...
title: "File tests/http.ts"
description: "Deep walkthrough of tests/http.ts in DG-ERP / Dhandho"
---

# File walkthrough: `tests/http.ts`
...
`tests/http.ts` is part of Dhandho (DG-ERP). Approximate size: **22 lines**.
...
**Exported names:** `getApp`, `api`, `authHeaders`
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `tests/http.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `getApp` before deleting. |
...
| Purpose | Symbol in `tests/http.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `authHeaders` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "http" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```
...
1. Open `tests/http.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `tests__http`*
