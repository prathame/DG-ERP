---
sidebar_label: "pg-manager.ts"
title: "File electron/onprem/pg-manager.ts"
description: "Deep walkthrough of electron/onprem/pg-manager.ts in DG-ERP / Dhandho"
---

# File walkthrough: `electron/onprem/pg-manager.ts`
...
`electron/onprem/pg-manager.ts` is part of Dhandho (DG-ERP). Approximate size: **109 lines**.
...
**Exported names:** `startPostgres`, `stopPostgres`
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `electron/onprem/pg-manager.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `getBinDir` before deleting. |
...
```ts
resolvePgPassword(dataDir: string, isFirstRun: boolean)
```
...
| Purpose | Symbol in `electron/onprem/pg-manager.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `resolvePgPassword` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `electron/onprem/pg-manager.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `ensureExecutable` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `electron/onprem/pg-manager.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `startPostgres` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
| Purpose | Symbol in `electron/onprem/pg-manager.ts`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `stopPostgres` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "pg-manager" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **109**. Large view/route files are refactor candidates.
...
1. Open `electron/onprem/pg-manager.ts` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `electron__onprem__pg-manager`*
