---
sidebar_label: "RewardRulesView.tsx"
title: "File src/features/masters/RewardRulesView.tsx"
description: "Deep walkthrough of src/features/masters/RewardRulesView.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/features/masters/RewardRulesView.tsx`
...
`src/features/masters/RewardRulesView.tsx` is part of Dhandho (DG-ERP). Approximate size: **131 lines**.
...
**Exported names:** `RewardRulesView`

**Classes:** _none_

## Functions (7 detected)

### Function: RewardRulesView

```ts
RewardRulesView({ onBack }: { onBack: ()
```

| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/features/masters/RewardRulesView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `RewardRulesView` before deleting. |
...
### Function: handleRedemptionSave

```ts
handleRedemptionSave(...)
```
...
| Purpose | Symbol in `src/features/masters/RewardRulesView.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `handleRedemptionSave` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "RewardRulesView" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **131**. Large view/route files are refactor candidates.
...
1. Open `src/features/masters/RewardRulesView.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__features__masters__rewardrulesview`*
