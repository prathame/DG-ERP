---
sidebar_label: "CustomCursor.tsx"
title: "File src/components/layout/CustomCursor.tsx"
description: "Deep walkthrough of src/components/layout/CustomCursor.tsx in DG-ERP / Dhandho"
---

# File walkthrough: `src/components/layout/CustomCursor.tsx`
...
`src/components/layout/CustomCursor.tsx` is part of Dhandho (DG-ERP). Approximate size: **56 lines**.
...
| Aspect | Detail |
| --- | --- |
| Purpose | Symbol in `src/components/layout/CustomCursor.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `CustomCursor` before deleting. |
...
| Purpose | Symbol in `src/components/layout/CustomCursor.tsx`. Open the source and read the body. |
| Parameters | See signature above. |
| What breaks if removed | Search the repo for `onOver` before deleting. |
| Security | If it touches auth, tenant_id, money, GST, or PII — treat as security-sensitive. |
| Performance | Watch for N+1 queries, unbounded loops, sync crypto, large JSON. |
| Alternatives | Inline (worse), extract shared helper (if duplicated), or use a standard library. |
...
rg -n "CustomCursor" --glob '!node_modules' -g '*.ts' -g '*.tsx'
```

## Performance impact

Line count **56**. Large view/route files are refactor candidates.
...
1. Open `src/components/layout/CustomCursor.tsx` in the IDE.
...
*Generated by scripts/generate-file-deepdives.mjs · slug: `src__components__layout__customcursor`*
