...
| `POST /api/onprem/heartbeat` | Public | Telemetry: version, disk, users |
| `POST /api/onprem/deactivate` | Public | Unbind |
| `POST /api/onprem/mark-applied` | Public | Ack settings push |
| `GET /api/onprem/tab-config` | **Localhost only** | Pull tab config |
| `POST /api/onprem/apply-settings` | **Localhost only** | Deep-merge pushed settings |
