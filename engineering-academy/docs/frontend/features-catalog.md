...
**Service phone UX (`isServicePhoneUx` — Offline Mobile **or** Service Cloud Capacitor + `businessType=service`):** **Products / Catalog inventory pill** and **Vendor-Customer Map** are hidden from Masters (no stock inventory; no mapping routes on that shell). **Price List** remains with **Catalog** + **Clients** scope tabs as the sellable rate book; invoice/quote lines pick Price List items (resolve) or custom free-text. Pill filter lives inline in `MastersView` (not extracted) — manual cases `cloud-mobile` #17 and `service-mobile` #5l / #5l2. Cloud manufacturer Masters keep Products → Inventory and Mapping unchanged.
