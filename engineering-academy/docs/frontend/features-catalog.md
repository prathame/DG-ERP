...
**Service phone UX (`isServicePhoneUx` — Offline Mobile **or** Cap Online + `businessType=service`):** **Products / Catalog inventory pill** and **Vendor-Customer Map** are hidden from Masters (no stock inventory; no mapping routes on that shell). **Price List** remains with **Catalog** + **Clients** scope tabs as the sellable rate book; invoice/quote lines pick Price List items (resolve) or custom free-text. Pill filter lives inline in `MastersView` (not extracted) — manual cases `cloud-mobile` #17 and `service-mobile` #5l / #5l2. Non-service Cap Online Masters follow the companion pack (`mobile_features`); desktop/browser manufacturer Masters keep Products → Inventory and Mapping unchanged.

**Service labels:** `businessTypeConfig` sets `labels.vendors` to **Clients** for `service` (manufacturer keeps **Vendors**). Service phone Masters pills, VendorMaster headers/FABs, invoice/quote party fields, Analytics “Outstanding Clients”, and Accounts party columns use that config — API paths stay `/vendors`.

**Client invoice hub (service only):** When `cfg.type === 'service'`, Masters → Clients (`VendorMasterView`) uses the **tile + detail hub** UX: tapping a client card (or a phone hub Client row with `initialVendorId`) opens that client’s Invoice Finance detail — outstanding / received / invoice list, **Record Payment** via `api.invoiceFinance`, and **New Invoice** (`CreateInvoiceModal` with party prefill). **Offline only:** with no outstanding bill, Record Payment stores a **client advance** (`partyKey`, no `invoiceId`); the next invoice auto-applies it and UI/print show **Advance payment** + **Outstanding**. Edit/Delete icons use `stopPropagation` and stay on the card. Back returns to the Clients list. Finance tab’s `InvoiceFinanceView` keeps the same APIs.

**Vendor list (non-service):** For retail / dealer / distribution / silver_casting / manufacturer and other non-service types, `VendorMasterView` is a **table list** of vendors (or Customers for dealer/retail labels) with edit/delete of vendor info only — **no tile cards**, **no invoice hub**, and **no New Invoice** on the vendor master.
...
Per-tenant configuration: user profile, password change, bill customization (logo, terms, bank details, **sale units** — Piece/Kg/Meter/custom — `bill_settings.bill_units`), **barcode & label templates** (visual designer under Bill Customization — see [barcode-label-templates.md](./barcode-label-templates.md)), and GST API credentials (NIC e-Invoice/e-Way Bill — encrypted at rest; see [../security/secrets.md](../security/secrets.md)). Sale units are picked on **invoice, quotation, and credit/debit-note** lines (decimal qty allowed). Inventory, barcode sales, purchases, and dispatch stay piece/box stock — they do not use this list.
