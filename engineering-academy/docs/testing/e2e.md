...
2. **For each business type** in `tests/e2e_by_type.py` (manufacturer, dealer, retail, service, `silver_casting`, `hotel_restaurant`): create a **fresh tenant** — real isolation, not shared fixtures, so one type's tests can never pollute another's. Vitest API coverage for hospitality: `tests/api/http-hospitality.test.ts`.
