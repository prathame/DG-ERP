...
├── android/  ios/          # Capacitor native projects (generated/synced, not hand-edited)
├── public/                 # Static assets served as-is (icons, manifest fallbacks)
├── docs/                   # Product-level docs shipped in the repo (MOBILE.md etc.)
...
flowchart TB
    ...
    Root --> Native["android/ + ios/ — Capacitor projects"]
    Root --> Infra["Infra configs — Docker, Render, Vite, Capacitor"]
...
│   ├── mobile/
│   │   ├── online/            # Capacitor bootstrap, invite/slug onboarding, heartbeat
│   │   └── offline/            # Mutation queue, GET cache, OfflineBanner
│   └── desktop/
...
│   ├── useEscapeKey.ts
│   └── offline/                # (Capacitor offline queue internals)
│
...
flowchart LR
    subgraph Matrix["platforms/ — a 2×2 matrix"]
        direction TB
        A["mobile · online<br/>(Capacitor bootstrap, invite onboarding)"]
...
These are standard Capacitor native project scaffolds, regenerated by `npx cap sync` from `dist-mobile/` + `capacitor.config.ts`. Treat manual edits here as fragile — anything that must persist across a `cap sync` belongs in `capacitor.config.ts` or a Capacitor plugin config, not a hand-edited native file.
...
└── cases/                     # Manual test-case markdown, one file per feature area (incl. pwa-mobile.md, super-admin.md)
```
...
| "Why does mobile behave differently" | `src/platforms/mobile/` |
...
| Mobile invite/heartbeat | `server/routes/mobile.ts`, `src/platforms/mobile/online/` |
...
- **`android/`/`ios/` are build artifacts of Capacitor**, not source you maintain directly.
...
>
> Expected answer: the feature's UI and business logic live in a new or existing `src/features/<name>/` folder, shared across every platform. Only the *connectivity/environment-specific* pieces — e.g., "queue this mutation if offline" or "this API origin differs on Capacitor" — belong in `src/platforms/mobile/offline/` or `src/platforms/shared/`. The feature component should call a platform-agnostic function (e.g. from `api.ts`, which already delegates to `platforms/mobile/offline` internally for queuing) rather than branching on `isMobileClient()` itself.
