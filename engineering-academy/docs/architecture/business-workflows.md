...
- Offline Mobile / Cap: Print uses system print / overlay (Save as PDF) with full bill HTML (`withPrintPagination`: thead repeat, avoid orphan rows/`print-end`). Cap bill PDFs use **one shared jsPDF builder** (`buildStandaloneInvoicePdfBlob`, `docType: 'invoice' | 'quotation'`): Bill Customization Save updates the ready **billSettings template**; WhatsApp fills doc fields (logo/signature via `addImage`, colors, terms, footer, tagline, signatory; **bank only for invoices**) — no html2pdf/html2canvas. Multi-page Cap PDF keeps a footer reserve clear of item rows (`ensureItemSpace`, wrap-aware heights), then repeats item-table headers on continued pages; totals/bank/sign draw into the reserve on the last page only. ~7s timeout → Dhandho + Cache FileProvider **file-only** Share; on timeout/fail → text summary. WhatsApp/PDF breadcrumbs + client logs are write-through to `localStorage`; Cap unexpected stop (dirty session, no clean pause) auto-saves a bug report under `Dhandho/bug-reports/` on next launch. **Follow-up:** optionally pre-bake PDF on Cap Invoice Save and reuse when fresh. Web browser still shares/downloads via html2pdf. **Cloud Electron desktop** is a separate branch (`shareElectronInvoiceWhatsApp`): same light jsPDF builder → IPC writes PDF to Downloads, copies file to clipboard when OS allows, reveals in Finder/Explorer, opens WhatsApp Desktop / `wa.me` with caption — not Cap Share and not html2pdf (html2pdf hung silently on Electron Mac). WhatsApp URL schemes cannot auto-attach files; user pastes or drags the PDF. Cap/mobile paths intentionally unchanged.
