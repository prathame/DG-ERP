...
- Offline Mobile / Cap: Print uses system print / overlay (Save as PDF) with full Tax Invoice HTML. Cap bill PDFs use **one shared jsPDF builder** (`buildStandaloneInvoicePdfBlob`): Bill Customization Save updates the ready **billSettings template**; WhatsApp loads that template and fills invoice-only fields (logo/signature via `addImage`, bank, colors, terms, footer, tagline, signatory) — no html2pdf/html2canvas. ~7s timeout → Dhandho/invoices + Cache FileProvider **file-only** Share; on timeout/fail → text summary. WhatsApp/PDF breadcrumbs + client logs are write-through to `localStorage`; Cap unexpected stop (dirty session, no clean pause) auto-saves a bug report under `Dhandho/bug-reports/` on next launch. **Follow-up:** optionally pre-bake PDF on Cap Invoice Save and reuse when fresh. Web still shares/downloads via html2pdf.
