...
| **Service Cloud seats** | Electron Cloud and/or Cap phone with **Online** mode | Same cloud tenant; device slots + company-wide session lock |
| **Service Mobile (Offline)** | Cap phone with **Offline** mode (`dist-service-phone`) | On-device PGlite; SA `DG-SM-` license + hard sync/backup |

**Unified Cap shell:** one Android + one iOS download. First launch picks Online or Offline **once** (separate auth/data — no switching, no shared login). Legacy separate Online Cap (`servicecloud`) app id is retired.

Offline is **service business type only**, separate from desktop on-prem licenses and from Online cloud seats. Do not mix license keys with cloud passwords.
