...
export function applyDateFilter(query, dateColumn, params, paramOffset) {
    let sql = '';
...
export async function logAudit(pool, tenantId, action, entityType, entityId, details, userId, userName) {
    const ctx = { tenantId, action, entityType, entityId, details, userId, userName };
    try {
        await pool.query('INSERT INTO audit_log (tenant_id, user_id, user_name, action, entity_type, entity_id, details) VALUES ($1, $2, $3, $4, $5, $6, $7)', [tenantId, userId ?? null, userName ?? null, action, entityType, entityId ?? null, details ?? null]);
...
export const mapProduct = (r) => ({
    ...
    barcodeUnitType: r.barcodeUnitType || r.barcode_unit_type || ((Number(r.pack_size) || 1) > 1 ? 'box' : 'piece'),
