...
export function getGstnPublicKey(mode: GstApiMode): string {
  ...
  const pem =
    (mode === 'production'
      ? process.env.GSTN_PRODUCTION_PUBLIC_KEY
      : process.env.GSTN_SANDBOX_PUBLIC_KEY) ||
...
export function buildIrnPayload(opts: {
  ...
  return {
    ...
    SellerDtls: {
      Gstin: opts.sellerGstin, LglNm: opts.sellerName,
      Addr1: opts.sellerAddr.substring(0, 100), Loc: opts.sellerAddr.substring(0, 50),
      Pin: Number(opts.sellerPin), Stcd: opts.sellerGstin.substring(0, 2),
    },
    BuyerDtls: {
      Gstin: buyerGst, LglNm: opts.buyerName,
      Pos: (buyerGst !== 'URP' ? buyerGst : opts.sellerGstin).substring(0, 2),
      Addr1: opts.buyerAddr.substring(0, 100), Loc: opts.buyerAddr.substring(0, 50),
      Pin: Number(opts.buyerPin), Stcd: (buyerGst !== 'URP' ? buyerGst : opts.sellerGstin).substring(0, 2),
    },
    ...
    ValDtls: {
      ...
      IgstVal: opts.totalIgst, TotInvVal: opts.grandTotal, RndOffAmt: 0,
    },
  };
}
...
export function buildEwbPayload(opts: {
  ...
  return {
    ...
    fromGstin: opts.sellerGstin, fromTrdName: opts.sellerName, fromAddr1: opts.sellerAddr, fromPlace: opts.sellerAddr,
    fromPincode: sellerPin, fromStateCode: Number(opts.sellerGstin.substring(0, 2)) || 24,
    actFromStateCode: Number(opts.sellerGstin.substring(0, 2)) || 24,
    toGstin: opts.buyerGstin || 'URP', toTrdName: opts.buyerName, toAddr1: opts.buyerAddr, toPlace: opts.buyerAddr,
    toPincode: buyerPin, toStateCode: Number((opts.buyerGstin || opts.sellerGstin).substring(0, 2)) || 24,
...
export class NicApiClient {
  ...
  private async authenticate(): Promise<{ authToken: string; sek: string }> {
    ...
    const encAppKey = crypto.publicEncrypt(
  ...
  async generateIrn(payload: ReturnType<typeof buildIrnPayload>): Promise<IrnResult> {
    ...
    return {
      irn: result.Irn, ackNo: result.AckNo, ackDt: result.AckDt,
      qrCode: result.QRCode || result.SignedQRCode,
      signedQrCode: result.SignedQRCode, ewbNo: result.EwbNo,
    };
  }
...
export async function loadGstCredentials(
  ...
  if (!row?.gst_api_client_id || !row?.gst_api_username) {
    return { ok: false, error: 'GST API not configured. Go to Settings → GST API (sandbox/production need credentials).' };
  }
  try {
...
export async function loadSellerPin(pool: import('pg').Pool, tenantId: string): Promise<string> {
  const row = (await pool.query(
    'SELECT gst_api_seller_pin FROM bill_settings WHERE tenant_id = $1',
    [tenantId]
