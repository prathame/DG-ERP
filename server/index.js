...
app.use((req, res, next) => {
    ...
    res.on('finish', () => {
        ...
        const tenant = req.headers['x-tenant-id']?.slice(0, 8) || '—';
...
app.use('/api/', async (req, res, next) => {
    ...
    try {
        ...
        if (decoded.tenantId) {
            ...
            const tenant = (await pool.query('SELECT status, subscription_ends_at, trial_ends_at FROM tenants WHERE id = $1', [decoded.tenantId])).rows[0];
...
app.get('/manifest.json', async (req, res) => {
    ...
    if (slug && slug !== 'admin' && slug !== 'privacy' && slug !== 'terms') {
        try {
            const tenant = (await pool.query('SELECT company_name FROM tenants WHERE slug = $1', [slug])).rows[0];
            if (tenant) {
