...
router.put('/api/settings/profile', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const row = (await pool.query('SELECT id, email, name, phone, address, role, company_name, auto_whatsapp, default_gst_rate, gst_number FROM users WHERE id = $1 AND tenant_id = $2', [userId, tenantId])).rows[0] as Record<string, unknown> | undefined;
    if (!row) return res.status(404).json({ error: 'User not found' });
