...
async function query(input: string, tenantId: string, tabConfig: TabConfig | null = null): Promise<ChatResponse> {
  ...
  const salesLbl = salesMeansDistribution
    ? tabLabel(tabConfig, 'distribution', 'Sales')
    : tabLabel(tabConfig, 'sales', 'Sales');
  ...
  if (/sales\s*today/.test(q)) {
    if (salesMeansDistribution) {
      ...
      const value = (
        (
          await pool.query(
            'SELECT COALESCE(SUM(COALESCE(billed_price, net_price, 0)), 0) as t FROM product_distribution pd JOIN products p ON pd.product_id = p.id AND p.tenant_id = $1 WHERE pd.distribution_date = $2 AND pd.tenant_id = $1',
            [tenantId, today],
          )
        ).rows[0] as { t: number }
      ).t;
    ...
    const count = (
      (
        await pool.query('SELECT COUNT(*) as c FROM product_sales WHERE purchase_date = $1 AND tenant_id = $2', [
          today,
          tenantId,
        ])
      ).rows[0] as { c: number }
    ...
    const revenue = (
      (
        await pool.query(
          'SELECT COALESCE(SUM(sale_price), 0) as t FROM product_sales WHERE purchase_date = $1 AND tenant_id = $2',
          [today, tenantId],
        )
      ).rows[0] as { t: number }
    ).t;
  ...
  if (/sales\s*(this\s*)?month/.test(q)) {
    ...
    const count = (
      (
        await pool.query(
          "SELECT COUNT(*) as c FROM product_sales WHERE to_char(purchase_date, 'YYYY-MM') = $1 AND tenant_id = $2",
          [month, tenantId],
        )
      ).rows[0] as { c: number }
    ).c;
    const revenue = (
      (
        await pool.query(
          "SELECT COALESCE(SUM(sale_price), 0) as t FROM product_sales WHERE to_char(purchase_date, 'YYYY-MM') = $1 AND tenant_id = $2",
          [month, tenantId],
        )
      ).rows[0] as { t: number }
    ).t;
  ...
  if (/recent\s*sales|last\s*sales|latest\s*sales/.test(q)) {
    const rows = (
      await pool.query(
        'SELECT ps.barcode, p.name, ps.customer_name, ps.purchase_date, ps.sale_price FROM product_sales ps JOIN products p ON ps.product_id = p.id AND p.tenant_id = $1 WHERE ps.tenant_id = $1 ORDER BY ps.purchase_date DESC LIMIT 5',
        [tenantId],
      )
    ).rows as { barcode: string; name: string; customer_name: string; purchase_date: string; sale_price: number }[];
  ...
  if (/top\s*product|best\s*sell|popular\s*product/.test(q)) {
    const rows = (
      await pool.query(
        'SELECT p.name, COUNT(ps.id) as sold, COALESCE(SUM(ps.sale_price), 0) as revenue FROM product_sales ps JOIN products p ON ps.product_id = p.id AND p.tenant_id = $1 WHERE ps.tenant_id = $1 GROUP BY ps.product_id, p.name ORDER BY sold DESC LIMIT 5',
        [tenantId],
      )
  ...
  if (/low\s*stock|stock\s*alert/.test(q)) {
    ...
    ).rows as { name: string; stock: number }[];
  ...
  if (/out\s*of\s*stock|zero\s*stock|no\s*stock/.test(q)) {
    const rows = (
      await pool.query(
        "SELECT p.name FROM products p LEFT JOIN product_inventory pi ON pi.product_id = p.id AND pi.status = 'InStock' AND pi.tenant_id = $1 WHERE p.tenant_id = $1 GROUP BY p.id, p.name HAVING COUNT(pi.id) = 0",
        [tenantId],
      )
    ).rows as { name: string }[];
  ...
  if (/total\s*(inventory|stock)|inventory\s*(count|summary)|how\s*many\s*product|stock\s*summary/.test(q)) {
    const total = (
      (await pool.query('SELECT COUNT(*) as c FROM product_inventory WHERE tenant_id = $1', [tenantId])).rows[0] as {
        c: number;
      }
    ).c;
    const inStock = (
      (
        await pool.query("SELECT COUNT(*) as c FROM product_inventory WHERE status = 'InStock' AND tenant_id = $1", [
          tenantId,
        ])
      ).rows[0] as { c: number }
    ).c;
    const distributed = (
      (
        await pool.query(
          "SELECT COUNT(*) as c FROM product_inventory WHERE status = 'Distributed' AND tenant_id = $1",
          [tenantId],
        )
      ).rows[0] as { c: number }
    ).c;
    const soldInv = (
      (
        await pool.query("SELECT COUNT(*) as c FROM product_inventory WHERE status = 'Sold' AND tenant_id = $1", [
          tenantId,
        ])
      ).rows[0] as { c: number }
    ).c;
    const productCount = (
      (await pool.query('SELECT COUNT(*) as c FROM products WHERE tenant_id = $1', [tenantId])).rows[0] as { c: number }
    ).c;
    return {
  ...
  if (/all\s*vendor|list\s*vendor|show\s*vendor|vendor\s*list/.test(q)) {
    const rows = (
      await pool.query("SELECT name, phone FROM vendors WHERE id != 'OWNER' AND tenant_id = $1 ORDER BY name", [
        tenantId,
      ])
    ).rows as { name: string; phone: string }[];
  ...
  if (/top\s*vendor|best\s*vendor/.test(q)) {
    const rows = (
      await pool.query(
        "SELECT v.name, v.total_sales as sold, v.total_reward_points as pts FROM vendors v WHERE v.id != 'OWNER' AND v.total_sales > 0 AND v.tenant_id = $1 ORDER BY v.total_sales DESC LIMIT 5",
        [tenantId],
      )
    ).rows as { name: string; sold: number; pts: number }[];
  ...
  if (/pending\s*payment|who\s*owe|outstanding|due\s*payment|vendor\s*balance/.test(q)) {
    ...
    ).rows as { name: string; total_val: number; paid: number }[];
  ...
  if (/all\s*customer|list\s*customer|show\s*customer|customer\s*list/.test(q)) {
    const rows = (
      await pool.query('SELECT name, phone FROM customers WHERE tenant_id = $1 ORDER BY name LIMIT 20', [tenantId])
    ).rows as { name: string; phone: string }[];
    if (rows.length === 0) return { text: 'No customers found.' };
    const total = (
      (await pool.query('SELECT COUNT(*) as c FROM customers WHERE tenant_id = $1', [tenantId])).rows[0] as {
        c: number;
      }
    ).c;
    const list = rows.map(r => `- ${r.name}${r.phone ? ` (${r.phone})` : ''}`).join('\n');
  ...
  if (/top\s*customer|best\s*customer|frequent\s*customer/.test(q)) {
    const rows = (
      await pool.query(
        'SELECT customer_name, COUNT(*) as purchases, COALESCE(SUM(sale_price), 0) as spent FROM product_sales WHERE tenant_id = $1 GROUP BY customer_name ORDER BY purchases DESC LIMIT 5',
        [tenantId],
      )
  ...
  if (/active\s*warrant|warranty\s*count|how\s*many\s*warrant/.test(q)) {
    const active = (
      (await pool.query("SELECT COUNT(*) as c FROM warranties WHERE status = 'Active' AND tenant_id = $1", [tenantId]))
        .rows[0] as { c: number }
    ).c;
    const expired = (
      (await pool.query("SELECT COUNT(*) as c FROM warranties WHERE status = 'Expired' AND tenant_id = $1", [tenantId]))
        .rows[0] as { c: number }
    ).c;
    const claimed = (
      (
        await pool.query("SELECT COUNT(*) as c FROM warranties WHERE status = 'Under Claim' AND tenant_id = $1", [
          tenantId,
        ])
      ).rows[0] as { c: number }
    ).c;
  ...
  if (/expir(ing|e)\s*warrant|warranty\s*expir/.test(q)) {
    ...
    const rows = (
      await pool.query(
        "SELECT barcode, customer_name, expiry_date FROM warranties WHERE status = 'Active' AND expiry_date BETWEEN $1 AND (CAST($1 AS DATE) + INTERVAL '30 days') AND tenant_id = $2 ORDER BY expiry_date LIMIT 10",
        [today, tenantId],
      )
    ).rows as { barcode: string; customer_name: string; expiry_date: string }[];
  ...
  if (/warranty\s*claim|under\s*claim|claimed\s*warrant/.test(q)) {
    const rows = (
      await pool.query(
        "SELECT barcode, customer_name, activation_date FROM warranties WHERE status = 'Under Claim' AND tenant_id = $1 ORDER BY activation_date DESC LIMIT 10",
        [tenantId],
      )
    ).rows as { barcode: string; customer_name: string; activation_date: string }[];
  ...
  if (/reward\s*point|total\s*reward|point\s*summary/.test(q)) {
    const earned = (
      (
        await pool.query("SELECT COALESCE(SUM(points), 0) as t FROM rewards WHERE type = 'Earned' AND tenant_id = $1", [
          tenantId,
        ])
      ).rows[0] as { t: number }
    ).t;
    const redeemed = (
      (
        await pool.query(
          "SELECT COALESCE(SUM(points), 0) as t FROM rewards WHERE type = 'Redeemed' AND tenant_id = $1",
          [tenantId],
        )
      ).rows[0] as { t: number }
    ).t;
  ...
  if (/top\s*earner|most\s*point|reward\s*leader/.test(q)) {
    const rows = (
      await pool.query(
        "SELECT name, total_reward_points as pts, total_sales FROM vendors WHERE id != 'OWNER' AND total_reward_points > 0 AND tenant_id = $1 ORDER BY total_reward_points DESC LIMIT 5",
        [tenantId],
      )
    ).rows as { name: string; pts: number; total_sales: number }[];
  ...
  if (/replacement|replaced\s*product|warranty\s*replace/.test(q)) {
    const count = (
      (await pool.query('SELECT COUNT(*) as c FROM product_replacements WHERE tenant_id = $1', [tenantId])).rows[0] as {
        c: number;
      }
    ).c;
    const rows = (
      await pool.query(
        'SELECT old_barcode, new_barcode, customer_name, replaced_date FROM product_replacements WHERE tenant_id = $1 ORDER BY replaced_date DESC LIMIT 5',
        [tenantId],
      )
    ).rows as { old_barcode: string; new_barcode: string; customer_name: string; replaced_date: string }[];
    if (count === 0) return { text: 'No product replacements recorded.' };
    const list = rows
      .map(r => `- ${r.customer_name}\n  ${r.old_barcode} -> ${r.new_barcode} (${r.replaced_date})`)
      .join('\n');
  ...
  if (/bank\s*account|bank\s*detail|list\s*bank|show\s*bank/.test(q)) {
    const rows = (
      await pool.query(
        'SELECT name, bank_name, account_number, branch, ifsc_code FROM banks WHERE tenant_id = $1 ORDER BY name',
        [tenantId],
      )
    ).rows as { name: string; bank_name: string; account_number: string; branch: string; ifsc_code: string }[];
    if (rows.length === 0) return { text: 'No bank accounts found.' };
    const list = rows
      .map(r => `- *${r.name}*\n  ${r.bank_name} | A/C: ${r.account_number}\n  ${r.branch} | IFSC: ${r.ifsc_code}`)
      .join('\n');
  ...
  if (/monthly\s*report|month\s*summary|this\s*month\s*report/.test(q)) {
    ...
    const distributedCount = (
      (
        await pool.query(
          "SELECT COUNT(*) as c FROM product_distribution WHERE to_char(distribution_date, 'YYYY-MM') = $1 AND tenant_id = $2",
          [month, tenantId],
        )
      ).rows[0] as { c: number }
    ).c;
    const payments = (
      (
        await pool.query(
          "SELECT COALESCE(SUM(amount), 0) as t FROM vendor_payments WHERE to_char(payment_date, 'YYYY-MM') = $1 AND tenant_id = $2",
          [month, tenantId],
        )
      ).rows[0] as { t: number }
    ).t;
  ...
  if (/distribution\s*summary|distributed\s*today|how\s*much\s*distributed/.test(q)) {
    ...
    const withVendor = (
      (
        await pool.query(
          "SELECT COUNT(*) as c FROM product_distribution WHERE status = 'Distributed' AND tenant_id = $1",
          [tenantId],
        )
      ).rows[0] as { c: number }
    ).c;
    const soldDist = (
      (
        await pool.query("SELECT COUNT(*) as c FROM product_distribution WHERE status = 'Sold' AND tenant_id = $1", [
          tenantId,
        ])
      ).rows[0] as { c: number }
    ).c;
    return {
      text: `*${distLbl} Summary*\n\n- Total dispatched: ${total}\n- With vendors: ${withVendor}\n- Sold by vendors: ${soldDist}`,
    };
  }
  ...
  if (barcodeMatch) {
    ...
    const inv = (
      await pool.query(
        'SELECT pi.barcode, pi.status, p.name as product_name, p.price FROM product_inventory pi JOIN products p ON pi.product_id = p.id AND p.tenant_id = $2 WHERE pi.barcode = $1 AND pi.tenant_id = $2',
        [barcode, tenantId],
      )
    ).rows[0] as { barcode: string; status: string; product_name: string; price: number } | undefined;
    if (inv) {
      let extra = '';
      if (inv.status === 'Distributed') {
        const dist = (
          await pool.query(
            'SELECT v.name, pd.distribution_date FROM product_distribution pd JOIN vendors v ON pd.vendor_id = v.id WHERE pd.barcode = $1 AND pd.tenant_id = $2',
            [barcode, tenantId],
          )
        ).rows[0] as { name: string; distribution_date: string } | undefined;
        extra = dist ? `\n- With: ${dist.name}\n- Since: ${dist.distribution_date}` : '';
      }
      if (inv.status === 'Sold') {
        const sale = (
          await pool.query(
            'SELECT customer_name, customer_phone, purchase_date, sale_price, vendor_id FROM product_sales WHERE barcode = $1 AND tenant_id = $2',
            [barcode, tenantId],
          )
        ).rows[0] as
          | {
      ...
      const warranty = (
        await pool.query(
          'SELECT status, expiry_date FROM warranties WHERE barcode = $1 AND tenant_id = $2 ORDER BY activation_date DESC LIMIT 1',
          [barcode, tenantId],
        )
  ...
  if (vendorRows.length === 1) {
    ...
    const totalVal = (
      (
        await pool.query(
          'SELECT COALESCE(SUM(COALESCE(pd.billed_price, pd.net_price, p.price)), 0) as t FROM product_distribution pd JOIN products p ON pd.product_id = p.id AND p.tenant_id = $2 WHERE pd.vendor_id = $1 AND pd.tenant_id = $2',
          [v.id, tenantId],
        )
      ).rows[0] as { t: number }
    ).t;
    ...
    const distCount = (
      (
        await pool.query('SELECT COUNT(*) as c FROM product_distribution WHERE vendor_id = $1 AND tenant_id = $2', [
          v.id,
          tenantId,
        ])
      ).rows[0] as { c: number }
    ...
    const soldCount = (
      (
        await pool.query(
          "SELECT COUNT(*) as c FROM product_distribution WHERE vendor_id = $1 AND status = 'Sold' AND tenant_id = $2",
          [v.id, tenantId],
        )
      ).rows[0] as { c: number }
    ).c;
    const withVendor = (
      (
        await pool.query(
          "SELECT COUNT(*) as c FROM product_distribution WHERE vendor_id = $1 AND status = 'Distributed' AND tenant_id = $2",
          [v.id, tenantId],
        )
      ).rows[0] as { c: number }
    ).c;
    const recentSales = (
      await pool.query(
        'SELECT p.name, ps.customer_name, ps.purchase_date FROM product_sales ps JOIN products p ON ps.product_id = p.id WHERE ps.vendor_id = $1 AND ps.tenant_id = $2 ORDER BY ps.purchase_date DESC LIMIT 3',
        [v.id, tenantId],
      )
    ).rows as { name: string; customer_name: string; purchase_date: string }[];
    const salesList =
  ...
  if (vendorRows.length > 1) {
    ...
    return {
      text: `Found ${vendorRows.length} vendors matching "${input}":\n\n${list}\n\nType the exact name for details.`,
    };
  }
  ...
  if (custRows.length === 1) {
    ...
    const purchaseRows = (
      await pool.query(
        'SELECT p.name, ps.purchase_date, ps.sale_price FROM product_sales ps JOIN products p ON ps.product_id = p.id WHERE (ps.customer_id = $1 OR (ps.customer_id IS NULL AND ps.customer_phone = $2)) AND ps.tenant_id = $3 ORDER BY ps.purchase_date DESC LIMIT 5',
        [c.id, c.phone ?? '', tenantId],
      )
    ).rows as { name: string; purchase_date: string; sale_price: number }[];
    const purchaseList =
    ...
    const vendor = (
      await pool.query(
        'SELECT v.name FROM customers c JOIN vendors v ON c.vendor_id = v.id WHERE c.id = $1 AND c.tenant_id = $2',
        [c.id, tenantId],
      )
    ).rows[0] as { name: string } | undefined;
    return {
  ...
  if (staffRows.length === 1) {
    ...
    const member = (
      await pool.query(
        'SELECT phone, role, salary FROM staff_members WHERE LOWER(name) = LOWER($1) AND tenant_id = $2',
        [s.staff_name, tenantId],
      )
    ).rows[0] as { phone?: string; role?: string; salary?: number } | undefined;
    const recent = (
      await pool.query(
        'SELECT amount, payment_date, payment_type FROM staff_payments WHERE staff_name = $1 AND tenant_id = $2 ORDER BY payment_date DESC LIMIT 5',
        [s.staff_name, tenantId],
      )
    ).rows as { amount: number; payment_date: string; payment_type: string }[];
