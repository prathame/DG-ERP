...
function rowToResponse(row) {
    return {
        ...
        footerText: row.footer_text || 'Powered by DG ERP Management',
    };
}
router.get('/api/settings/bill', authMiddleware, async (req, res) => {
    try {
...
router.put('/api/settings/bill', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(400).json({ error: 'Tenant ID required' });
        ...
        if (requestBody.logoBase64) {
            if (!/^data:image\/(jpeg|png|webp|gif);base64,/.test(requestBody.logoBase64) || requestBody.logoBase64.length > 700000) {
                return res.status(400).json({ error: 'Logo must be JPEG, PNG, WebP or GIF under 500KB' });
            }
        }
        if (requestBody.signatureBase64) {
            if (!/^data:image\/(jpeg|png|webp|gif);base64,/.test(requestBody.signatureBase64) || requestBody.signatureBase64.length > 700000) {
                return res.status(400).json({ error: 'Signature must be JPEG, PNG, WebP or GIF under 500KB' });
            }
        }
        if (requestBody.signatureBase64 && (!requestBody.signatureBase64.startsWith('data:image/') || requestBody.signatureBase64.length > 700000)) {
