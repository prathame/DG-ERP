...
router.get('/api/dashboard/stats', async (req: AuthRequest, res) => {
  try {
    ...
    const [scalar, outstanding, todayDist, lowStock, topProducts] = await Promise.all([
      scalarQ, outstandingQ, todayDistQ, lowStockQ, topProductsQ,
    ]);
    ...
    res.json({
      ...
      todayDistributionValue: Number(todayDist.rows[0]?.total) || 0,
      expiringWarranties:     Number(s.expiring_warranties)    || 0,
      lowStockProducts: (lowStock.rows as { id: string; name: string; stock: number }[])
...
router.get('/api/analytics/overview', async (req, res) => {
  try {
    ...
    res.json({
      ...
      topVendors: (vendorSummary.rows as {id:string;name:string;phone:string;distributed:string;paid:string}[]).map(r => ({
        vendorId: r.id, vendorName: r.name, balance: Number(r.distributed) - Number(r.paid),
      })),
...
router.get('/api/dashboard/rewards-summary', async (req, res) => {
  try {
    ...
    res.json({
      vendors: (vendorRows.rows as { id: string; name: string; products_sold: number; total_reward_points: number }[]).map(v => ({
        id: v.id, name: v.name,
      ...
      topProducts: (productSales.rows as { product_name: string; sold: number }[]).map(p => ({
        name: p.product_name, sold: Number(p.sold) || 0,
      })),
    });
  } catch (err) {
    console.error(`💥 ${req.method} ${req.originalUrl} failed:`, (err as Error).message);
