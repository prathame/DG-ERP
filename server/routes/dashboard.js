...
router.get('/api/dashboard/stats', async (req, res) => {
    try {
        ...
        const [revenue, warranties, pendingClaims, rewardsEarned, totalProducts, distributed, sold, vendorRewards, withAdmin, withVendors, totalInventory, availableInInventory, todaySales, thisMonthSales, lastMonthSales, thisMonthRevenue, lastMonthRevenue, totalVendorReceived, totalVendorOutstanding, todayCollections, todayRevenue, todayDistributionValue] = await Promise.all([
            pool.query("SELECT COALESCE(SUM(sale_price), 0) as total FROM product_sales WHERE tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as count FROM warranties WHERE status = 'Active' AND tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as count FROM warranties WHERE status = 'Under Claim' AND tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COALESCE(SUM(points), 0) as total FROM rewards WHERE type = 'Earned' AND tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query('SELECT COUNT(*) as count FROM products WHERE tenant_id = $1', [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as count FROM product_distribution WHERE status = 'Distributed' AND tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as count FROM product_distribution WHERE status = 'Sold' AND tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query('SELECT COALESCE(SUM(total_reward_points), 0) as total FROM vendors WHERE tenant_id = $1', [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as count FROM product_inventory WHERE status = 'InStock' AND tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as count FROM product_distribution WHERE status = 'Distributed' AND tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as count FROM product_inventory WHERE tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query('SELECT COALESCE(SUM(stock), 0) as total FROM products WHERE tenant_id = $1', [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as c FROM product_sales WHERE purchase_date = $1 AND tenant_id = $2", [today, tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as c FROM product_sales WHERE to_char(purchase_date, 'YYYY-MM') = $1 AND tenant_id = $2", [thisMonth, tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COUNT(*) as c FROM product_sales WHERE to_char(purchase_date, 'YYYY-MM') = $1 AND tenant_id = $2", [lastMonth, tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COALESCE(SUM(sale_price), 0) as t FROM product_sales WHERE to_char(purchase_date, 'YYYY-MM') = $1 AND tenant_id = $2", [thisMonth, tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COALESCE(SUM(sale_price), 0) as t FROM product_sales WHERE to_char(purchase_date, 'YYYY-MM') = $1 AND tenant_id = $2", [lastMonth, tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COALESCE(SUM(amount), 0) as total FROM vendor_payments WHERE tenant_id = $1", [tenantId]).then(r => r.rows[0]),
            pool.query(`SELECT COALESCE(SUM(COALESCE(pd.billed_price, pd.net_price, p.price)), 0) - COALESCE((SELECT SUM(amount) FROM vendor_payments WHERE tenant_id = $1), 0) as outstanding FROM product_distribution pd JOIN products p ON pd.product_id = p.id AND p.tenant_id = $1 WHERE pd.tenant_id = $1`, [tenantId]).then(r => r.rows[0]),
            pool.query("SELECT COALESCE(SUM(amount), 0) as total FROM vendor_payments WHERE tenant_id = $1 AND payment_date = $2", [tenantId, today]).then(r => r.rows[0]),
            pool.query("SELECT COALESCE(SUM(sale_price), 0) as total FROM product_sales WHERE tenant_id = $1 AND purchase_date = $2", [tenantId, today]).then(r => r.rows[0]),
            pool.query(`SELECT COALESCE(SUM(COALESCE(pd.billed_price, pd.net_price, p.price)), 0) as total FROM product_distribution pd JOIN products p ON pd.product_id = p.id AND p.tenant_id = $1 WHERE pd.tenant_id = $1 AND pd.distribution_date = $2`, [tenantId, today]).then(r => r.rows[0]),
        ]);
...
router.get('/api/dashboard/money', async (req, res) => {
    try {
        ...
        const dateFilter = (col) => from && to ? `AND ${col} BETWEEN $2 AND $3` : from ? `AND ${col} >= $2` : '';
        const params = (extra) => from && to ? [tenantId, from, to, ...extra] : from ? [tenantId, from, ...extra] : [tenantId, ...extra];
...
router.get('/api/analytics/recent-activity', async (req, res) => {
    ...
    `, [tenantId])).rows;
        ...
        const vendorMap = {};
        if (vendorIds.length) {
            const vRows = (await pool.query('SELECT id, name FROM vendors WHERE tenant_id = $1 AND id = ANY($2)', [tenantId, vendorIds])).rows;
...
router.get('/api/analytics/overview', async (req, res) => {
    try {
        ...
        const dateFilter = (col) => from && to ? `AND ${col} BETWEEN $2 AND $3` : from ? `AND ${col} >= $2` : '';
        const params = (extra) => from && to ? [tenantId, from, to, ...extra] : from ? [tenantId, from, ...extra] : [tenantId, ...extra];
        const [collections, salesRev, invoiceRev, distribution, expenses, outstanding, invoiceOutstanding, activityRows, vendorSummary, counts] = await Promise.all([
            pool.query(`SELECT COALESCE(SUM(amount),0) as v FROM vendor_payments WHERE tenant_id=$1 ${dateFilter('payment_date')}`, params([])).then(r => Number(r.rows[0].v) || 0),
        ...
        const vendorIds = [...new Set(activityRows.rows.filter((r) => r.type === 'payment' || r.type === 'distribution').map((r) => r.label))];
        const vendorMap = {};
        if (vendorIds.length) {
        ...
        res.json({
            ...
            recentActivity: activityRows.rows.map((r) => ({ ...r, amount: Number(r.amount) || 0, label: (r.type === 'payment' || r.type === 'distribution') ? (vendorMap[r.label] || r.label) : r.label })),
            topVendors: vendorSummary.rows.map((r) => ({ vendorId: r.id, vendorName: r.name, balance: Number(r.distributed) || 0 - Number(r.paid) || 0 })),
...
router.get('/api/dashboard/vendor/:vendorId', async (req, res) => {
    try {
        ...
        const vendor = (await pool.query('SELECT * FROM vendors WHERE id = $1 AND tenant_id = $2', [vendorId, tenantId])).rows[0];
        if (!vendor)
