...
router.get('/api/mapping/vendors-with-customers', async (req: AuthRequest, res) => {
  try {
    ...
    `, vid ? [tenantId, vid] : [tenantId])).rows as { id: string; vendor_name: string; contact_person: string; phone: string; customer_id: string | null; customer_name: string | null; customer_phone: string | null; customer_email: string | null }[];
    ...
    const directCustomers = vid
      ? []
      : ((await pool.query('SELECT id, name, phone, email FROM customers WHERE vendor_id IS NULL AND tenant_id = $1 ORDER BY name', [tenantId])).rows as { id: string; name: string; phone: string; email: string }[]);
