...
router.post('/api/vendors/bulk', async (req, res) => {
    ...
    try {
        ...
        const slug = (await client.query('SELECT slug FROM tenants WHERE id = $1', [tenantId])).rows[0]?.slug;
        ...
        for (let i = 0; i < vendors.length; i++) {
            ...
            const id = uid('V');
            await client.query('INSERT INTO vendors (id, tenant_id, name, contact_person, phone, email, address, gst_number) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)', [id, tenantId, v.name.trim(), v.contactPerson || null, v.phone?.trim() || null, v.email || null, v.address || null, v.gstNumber || null]);
            if (portalEnabled && v.email && v.email.includes('@')) {
                ...
                if (!existing) {
                    ...
                    await client.query(`INSERT INTO users (id, tenant_id, email, password_hash, name, phone, address, role, company_name, permissions, vendor_id) VALUES ($1,$2,$3,$4,$5,$6,$7,'Vendor',$8,$9,$10)`, [userId, tenantId, v.email, hashPassword(pw), v.contactPerson || v.name, v.phone || null, v.address || null, v.name, perms, id]);
        ...
        res.status(500).json({ error: e.message || 'Import failed — no vendors were added' });
    }
    finally {
        client.release();
    }
...
router.post('/api/vendors', async (req, res) => {
    try {
        ...
        const gstNum = req.body.gstNumber;
        if (gstNum && !isValidGstin(gstNum))
            return res.status(400).json({ error: 'Invalid GSTIN — must be 15 characters (e.g. 24AABCT1332L1ZS)' });
        const duplicate = (await pool.query('SELECT id, name FROM vendors WHERE tenant_id = $1 AND (LOWER(name) = LOWER($2) OR (email IS NOT NULL AND email != \'\' AND LOWER(email) = LOWER($3)))', [tenantId, name.trim(), email || ''])).rows[0];
        if (duplicate)
            return res.status(400).json({ error: `Vendor "${duplicate.name}" already exists` });
        const id = uid('V');
        await pool.query('INSERT INTO vendors (id, tenant_id, name, contact_person, phone, email, address, gst_number) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)', [id, tenantId, name.trim(), contactPerson, phone?.trim() || null, email, address, req.body.gstNumber || null]);
        ...
        if (portalEnabled && email && typeof email === 'string' && email.includes('@')) {
            ...
            if (!existing) {
                ...
                const userId = uid('U');
                const perms = JSON.stringify({ dashboard: 'view', sales: 'hidden', distribution: 'view', inventory: 'hidden', purchases: 'hidden', quotations: 'hidden', orders: 'hidden', finance: 'view', accounts: 'hidden', warranty: 'hidden', replacements: 'hidden', rewards: 'hidden', settings: 'hidden' });
                await pool.query(`INSERT INTO users (id, tenant_id, email, password_hash, name, phone, address, role, company_name, permissions, vendor_id)
           VALUES ($1, $2, $3, $4, $5, $6, $7, 'Vendor', $8, $9, $10)`, [userId, tenantId, email, hashPassword(defaultPassword), contactPerson || name || '', phone || null, address || null, name || null, perms, id]);
...
router.put('/api/vendors/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query('UPDATE vendors SET name=COALESCE($1,name), contact_person=COALESCE($2,contact_person), phone=COALESCE($3,phone), email=COALESCE($4,email), address=COALESCE($5,address), gst_number=COALESCE($8,gst_number) WHERE id=$6 AND tenant_id=$7', [name, contactPerson, phone?.trim() || null, email, address, id, tenantId, gstNumber ?? null]);
...
router.delete('/api/vendors/:id', async (req, res) => {
    try {
        ...
        if (hasDistributions)
            return res.status(400).json({ error: 'Cannot delete vendor with existing distributions. Remove distributions first.' });
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
