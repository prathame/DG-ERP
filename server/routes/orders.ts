...
router.get('/api/orders/:id', async (req, res) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string;
    if (!tenantId) return res.status(401).json({ error: 'Tenant ID required' });
    const r = (await pool.query('SELECT * FROM orders WHERE id = $1 AND tenant_id = $2', [req.params.id, tenantId])).rows[0] as Record<string, unknown> | undefined;
...
router.post('/api/orders', async (req, res) => {
  try {
    ...
    const resolvedItems: { productId: string; productName: string; quantity: number; price: number; discountPercent: number; withGst: boolean; lineNet: number; lineGst: number; lineTotal: number }[] = [];
    let subtotal = 0;
    let gstAmount = 0;

    for (const item of items) {
      const product = (await pool.query('SELECT name, price FROM products WHERE id = $1 AND tenant_id = $2', [item.productId, tenantId])).rows[0] as { name: string; price: number } | undefined;
    ...
    await pool.query(
      ...
      [id, tenantId, orderNum, vendorId || null, vName, customerName || vName, customerPhone || null, customerGstNumber || null,
...
router.post('/api/orders/:id/fulfill', async (req, res) => {
  try {
    ...
    try {
      ...
      for (const item of items) {
        const qty = Number(item.quantity) || 1;
        const product = (await client.query('SELECT id, name, price, pack_size FROM products WHERE id = $1 AND tenant_id = $2', [item.productId, tenantId])).rows[0] as { id: string; name: string; price: number; pack_size: number } | undefined;
        if (!product) { await client.query('ROLLBACK'); return res.status(404).json({ error: `Product not found: ${item.productId}` }); }

        const unitTypeRow = (await client.query("SELECT unit_type FROM product_inventory WHERE product_id = $1 AND status = 'InStock' AND tenant_id = $2 LIMIT 1", [product.id, tenantId])).rows[0] as { unit_type: string } | undefined;
        ...
        if (invRows.length < qty) { await client.query('ROLLBACK'); return res.status(400).json({ error: `Insufficient stock for ${product.name}. Available: ${invRows.length}, requested: ${qty}` }); }
        ...
        for (const inv of invRows) {
          ...
          await client.query(
            ...
            [distId, batchId, product.id, inv.barcode, order.vendor_id, date, 'Distributed', disc, netPrice, gstApplied === 1, billedPrice, tenantId]
          );
