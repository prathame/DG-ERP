...
router.get('/api/vendor-finance/summary', async (req: AuthRequest, res) => {
  try {
    ...
    const vendors = (
      ...
      ORDER BY v.name
    `,
        vendorScopeId(req) ? [tenantId, vendorScopeId(req)] : [tenantId],
      )
    ).rows as {
      id: string;
      name: string;
    ...
    const reminders = (
      await pool.query(
        'SELECT vendor_id, enabled, reminder_days, last_reminder_date FROM vendor_reminder_settings WHERE tenant_id = $1',
        [tenantId],
      )
    ).rows as { vendor_id: string; enabled: boolean; reminder_days: number; last_reminder_date: string | null }[];
...
router.get('/api/vendor-finance/:vendorId', async (req: AuthRequest, res) => {
  try {
    ...
    const vendor = (
      await pool.query(
        'SELECT id, name, phone, email, address, contact_person FROM vendors WHERE id = $1 AND tenant_id = $2',
        [vendorId, tenantId],
      )
    ...
    const totalValue = (
      await pool.query(
        `SELECT COALESCE(SUM(${DISTRIBUTION_BILL_UNIT_SQL}), 0) as total FROM product_distribution pd JOIN products p ON pd.product_id = p.id WHERE pd.vendor_id = $1 AND pd.tenant_id = $2`,
        [vendorId, tenantId],
      )
    ).rows[0] as { total: number };
    const totalPaid = (
      await pool.query(
        'SELECT COALESCE(SUM(amount), 0) as total FROM vendor_payments WHERE vendor_id = $1 AND tenant_id = $2',
        [vendorId, tenantId],
      )
    ).rows[0] as { total: number };
    const payments = (
    ...
    const reminder = (
      await pool.query(
        'SELECT enabled, reminder_days, last_reminder_date FROM vendor_reminder_settings WHERE vendor_id = $1 AND tenant_id = $2',
        [vendorId, tenantId],
      )
    ).rows[0] as { enabled: boolean; reminder_days: number; last_reminder_date: string | null } | undefined;
    ...
    res.json({
      ...
      reminder: reminder
        ? { enabled: !!reminder.enabled, days: reminder.reminder_days, lastSent: reminder.last_reminder_date }
        : { enabled: false, days: 7, lastSent: null },
    });
  } catch (err) {
    return handleApiError(req, res, err);
...
router.post('/api/vendor-finance/:vendorId/payments', blockVendors, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    if (batchId) {
      ...
      ).rows as { batch_id: string; bill_value: number; paid: number }[];
...
router.put('/api/vendor-finance/:vendorId/reminder', blockVendors, async (req: AuthRequest, res) => {
  try {
    ...
    const row = (
      await pool.query('SELECT * FROM vendor_reminder_settings WHERE vendor_id = $1 AND tenant_id = $2', [
        vendorId,
        tenantId,
      ])
...
router.post('/api/vendor-finance/bank-statement/preview', blockVendors, async (req: AuthRequest, res) => {
  try {
    ...
    const { transactions } = req.body as {
      transactions: { date: string; description: string; amount: number; reference?: string }[];
    };
    ...
    ).rows as { batch_id: string; vendor_id: string; date: string; bill_value: string }[];
    ...
    const vendorBatches: Record<
      string,
      { batchId: string; date: string; billValue: number; paid: number; balance: number }[]
    > = {};
    for (const b of batchRows) {
      ...
      vendorBatches[b.vendor_id].push({
        batchId: b.batch_id,
        date: b.date,
        billValue: Number(b.bill_value),
        paid,
        balance,
      });
    }
...
router.post('/api/vendor-finance/bank-statement/apply', blockVendors, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    for (const p of payments) {
      ...
      const vendor = (
        await client.query('SELECT id FROM vendors WHERE id = $1 AND tenant_id = $2 FOR UPDATE', [p.vendorId, tenantId])
      ).rows[0];
      if (!vendor) {
    ...
    await logAudit(
      ...
      'payment',
      importBatchId,
      `${count} payments applied from bank statement`,
    );
    res.json({ applied: count, importBatchId, skipped });
  } catch (e) {
    await client.query('ROLLBACK');
    return handleApiError(req, res, e, 'Apply payments failed', { publicMessage: 'Failed to apply payments' });
  } finally {
    client.release();
  }
});
