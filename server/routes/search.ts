  ...
  try {
    ...
    if (vid) {
      ...
      const [products, customers, challans] = await Promise.all([
        ...
        `,
            [tenantId, like, vid, limit],
          )
          .then(r => r.rows as { id: string; name: string; price: number; stock: number }[]),
        pool
          .query(
            `
        ...
        `,
            [like, tenantId, vid, limit],
          )
          .then(r => r.rows as { id: string; name: string; phone: string; email: string }[]),
        pool
          .query(
            `
        ...
        `,
          ...
          .then(
            r => r.rows as { batch_id: string; vendor_name: string; distribution_date: string; total_units: number }[],
          ),
      ]);
      return res.json({
        ...
        challans: challans.map(c => ({
          ...
          date: c.distribution_date,
          units: Number(c.total_units),
    ...
    const [products, customers, vendors, barcodeResults, challans, staff] = await Promise.all([
      ...
      `,
        ...
        .then(r => r.rows as { id: string; name: string; contact_person: string; phone: string }[]),
      pool
        .query(
          `
      ...
      `,
        ...
        .then(r => r.rows as { barcode: string; product_name: string; product_id: string; status: string }[]),
      pool
        .query(
          `
        SELECT COALESCE(pd.batch_id, pd.id) as batch_id, v.name as vendor_name,
      ...
      `,
        ...
        .then(r => r.rows as { staff_name: string; total_paid: number; payments: number; last_payment: string }[]),
    ]);
