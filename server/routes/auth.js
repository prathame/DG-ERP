...
router.post('/api/auth/signup', async (req, res) => {
    try {
        ...
        const existingAdmins = (await pool.query("SELECT COUNT(*) as c FROM users WHERE tenant_id = $1 AND role IN ('Admin', 'Super Admin')", [tenantId])).rows[0];
        if (Number(existingAdmins.c) > 0)
            return res.status(403).json({ error: 'Signup is disabled. Contact your admin to create accounts.' });
    ...
    `, [id, emailLower, passwordHash, name ?? '', phone ?? null, address ?? null, 'Admin', companyName ?? null, tenantId]);
        const row = (await pool.query('SELECT id, email, name, phone, address, role, company_name FROM users WHERE id = $1 AND tenant_id = $2', [id, tenantId])).rows[0];
        const tenant = (await pool.query('SELECT slug FROM tenants WHERE id = $1', [tenantId])).rows[0];
...
router.post('/api/auth/login', async (req, res) => {
    ...
    `, [email.trim()])).rows[0];
        ...
        await logAudit(pool, row.tenant_id, 'LOGIN', 'user', row.id, `User logged in: ${row.email}`, row.id, row.name);
        res.json({
            token,
            tenantId: row.tenant_id,
            ...
            planName: await (async () => {
                ...
                if (!pid)
                    return row.tenant_status === 'trial' ? 'Free Trial' : 'Standard';
                const planRow = (await pool.query('SELECT name FROM plans WHERE id = $1', [pid])).rows[0];
                if (planRow)
                    return planRow.name;
                if (pid === 'TRIAL' || row.tenant_status === 'trial')
                    return 'Free Trial';
...
router.get('/api/settings/profile', authMiddleware, async (req, res) => {
    try {
...
router.put('/api/settings/profile', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(401).json({ error: 'Tenant ID required' });
...
router.put('/api/settings/change-password', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(401).json({ error: 'Tenant ID required' });
        ...
        if (newPassword.length < 8)
            return res.status(400).json({ error: 'Password must be at least 8 characters' });
        const user = (await pool.query('SELECT id, password_hash FROM users WHERE id = $1 AND tenant_id = $2', [userId, tenantId])).rows[0];
        if (!user)
            return res.status(404).json({ error: 'User not found' });
...
router.post('/api/auth/forgot-password', async (req, res) => {
    try {
        ...
        const user = (await pool.query('SELECT u.id, u.email, u.tenant_id FROM users u WHERE LOWER(u.email) = LOWER($1)', [email])).rows[0];
        ...
        await pool.query('INSERT INTO password_reset_tokens (id, email, tenant_id, token, expires_at) VALUES ($1, $2, $3, $4, $5)', [uid('PRT'), email, user.tenant_id, token, expiresAt]);
...
router.post('/api/auth/reset-password', async (req, res) => {
    try {
        ...
        if (newPassword.length < 8)
            return res.status(400).json({ error: 'Password must be at least 8 characters' });
        const resetToken = (await pool.query('SELECT * FROM password_reset_tokens WHERE token = $1 AND used = false AND expires_at > NOW()', [token])).rows[0];
        if (!resetToken)
        ...
        await logAudit(pool, resetToken.tenant_id, 'PASSWORD_RESET', 'user', null, `Password reset for ${resetToken.email}`, undefined, resetToken.email);
...
router.put('/api/admin/reset-user-password', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        if (!tenantId)
            return res.status(401).json({ error: 'Tenant ID required' });
        ...
        if (newPassword.length < 8)
            return res.status(400).json({ error: 'Password must be at least 8 characters' });
        const user = (await pool.query('SELECT id, email, name FROM users WHERE id = $1 AND tenant_id = $2', [userId, tenantId])).rows[0];
        if (!user)
            return res.status(404).json({ error: 'User not found' });
        const newHash = bcrypt.hashSync(newPassword, 12);
        await pool.query('UPDATE users SET password_hash = $1, password_changed_at = NOW() WHERE id = $2 AND tenant_id = $3', [newHash, userId, tenantId]);
