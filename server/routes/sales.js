...
router.get('/api/sales/validate/:barcode', async (req, res) => {
    ...
    `, [barcode, tenantId])).rows[0];
        if (dist) {
            if (restrictToVendorId && dist.vendor_id !== restrictToVendorId)
                return res.json({ valid: false, error: 'Barcode not assigned to your vendor' });
            return res.json({
                valid: true,
        ...
        if (assigned)
            return res.json({ valid: false, error: 'Product already sold or returned' });
    ...
    `, [barcode, tenantId])).rows[0];
        ...
        if (exists)
            return res.json({ valid: false, error: 'Product already sold or distributed' });
    ...
    `, [barcode, tenantId])).rows[0] : null;
        ...
        try {
            ...
            const existingCustomer = phoneNorm
                ? (await client.query("SELECT id FROM customers WHERE TRIM(COALESCE(phone, '')) = $1 AND TRIM(COALESCE(name, '')) = $2 AND tenant_id = $3", [phoneNorm, String(customerName ?? '').trim(), tenantId])).rows[0]
                : undefined;
            let customerId;
            if (existingCustomer) {
                ...
                customerId = uid('C');
                await client.query('INSERT INTO customers (id, tenant_id, name, phone, email, address, vendor_id) VALUES ($1, $2, $3, $4, $5, $6, $7)', [customerId, tenantId, customerName, customerPhone, customerEmail || null, null, vendorId]);
            }
      ...
      `, [id, tenantId, barcode, productId, vendorId, customerId, customerName, customerPhone, customerEmail || null, date, points, priceVal]);
            if (dist)
                await client.query("UPDATE product_distribution SET status = 'Sold' WHERE barcode = $1 AND tenant_id = $2", [barcode, tenantId]);
            ...
            const productName = (await client.query('SELECT name FROM products WHERE id = $1 AND tenant_id = $2', [productId, tenantId])).rows[0];
      ...
      `, [warrantyId, tenantId, productId, barcode, customerName, customerPhone, activationDate, expiryDate]);
        ...
        const row = (await pool.query('SELECT * FROM product_sales WHERE id = $1 AND tenant_id = $2', [id, tenantId])).rows[0];
        res.status(201).json({
            id: row.id,
            barcode: row.barcode,
...
router.get('/api/sales', async (req, res) => {
    try {
        ...
        if (typeof vendorId === 'string' && vendorId) {
            where += ` AND ps.vendor_id = $${params.length + 1}`;
            params.push(vendorId);
        }
        where += applyDateFilter(req.query, 'ps.purchase_date', params);
        const countParams = [...params];
        const total = (await pool.query(`SELECT COUNT(*) as c FROM product_sales ps ${where}`, countParams)).rows[0].c;
...
router.get('/api/sales/:id/bill', async (req, res) => {
    ...
    `, [id, tenantId])).rows[0];
        ...
        const warranty = (await pool.query('SELECT activation_date, expiry_date, status FROM warranties WHERE barcode = $1 AND tenant_id = $2 ORDER BY activation_date DESC LIMIT 1', [sale.barcode, tenantId])).rows[0];
        ...
        let vendorFinance = null;
        const vid = sale.vendor_id;
        if (vid && vid !== 'OWNER') {
            const totalValue = (await pool.query('SELECT COALESCE(SUM(COALESCE(pd.billed_price, pd.net_price, p.price)), 0) as t FROM product_distribution pd JOIN products p ON pd.product_id = p.id AND p.tenant_id = $2 WHERE pd.vendor_id = $1 AND pd.tenant_id = $2', [vid, tenantId])).rows[0];
            const totalPaid = (await pool.query('SELECT COALESCE(SUM(amount), 0) as t FROM vendor_payments WHERE vendor_id = $1 AND tenant_id = $2', [vid, tenantId])).rows[0];
