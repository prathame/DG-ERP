...
import { uid, isValidPhone, isValidEmail } from '../utils/helpers';
const router = Router();
...
router.post('/api/customers', async (req, res) => {
    try {
        ...
        const id = uid('C');
        await pool.query('INSERT INTO customers (id, tenant_id, name, phone, email, address, vendor_id) VALUES ($1, $2, $3, $4, $5, $6, $7)', [id, tenantId, name.trim(), phone, email, address, vendorId || null]);
...
router.put('/api/customers/:id', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const { name, phone, email, address, vendorId } = req.body;
        const result = await pool.query('UPDATE customers SET name=COALESCE($1,name), phone=COALESCE($2,phone), email=COALESCE($3,email), address=COALESCE($4,address), vendor_id=$5 WHERE id=$6 AND tenant_id=$7', [name, phone, email, address, vendorId === '' || vendorId === undefined ? null : vendorId, id, tenantId]);
...
router.get('/api/customers/:id/purchases', async (req, res) => {
    try {
        ...
        if (!tenantId)
            return res.status(401).json({ error: 'Tenant ID required' });
        const { id } = req.params;
        const customer = (await pool.query('SELECT id, phone FROM customers WHERE id = $1 AND tenant_id = $2', [id, tenantId])).rows[0];
        if (!customer)
            return res.json([]);
