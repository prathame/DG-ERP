...
router.get('/api/vendor-finance/summary', async (req, res) => {
    ...
    `, [tenantId])).rows;
        const reminders = (await pool.query('SELECT vendor_id, enabled, reminder_days, last_reminder_date FROM vendor_reminder_settings WHERE tenant_id = $1', [tenantId])).rows;
...
router.get('/api/vendor-finance/:vendorId', async (req, res) => {
    try {
        ...
        const vendor = (await pool.query('SELECT id, name, phone, email, address, contact_person FROM vendors WHERE id = $1 AND tenant_id = $2', [vendorId, tenantId])).rows[0];
        if (!vendor)
            return res.status(404).json({ error: 'Vendor not found' });
        const totalValue = (await pool.query(`SELECT COALESCE(SUM(${DISTRIBUTION_BILL_UNIT_SQL}), 0) as total FROM product_distribution pd JOIN products p ON pd.product_id = p.id WHERE pd.vendor_id = $1 AND pd.tenant_id = $2`, [vendorId, tenantId])).rows[0];
        const totalPaid = (await pool.query('SELECT COALESCE(SUM(amount), 0) as total FROM vendor_payments WHERE vendor_id = $1 AND tenant_id = $2', [vendorId, tenantId])).rows[0];
        const payments = (await pool.query('SELECT * FROM vendor_payments WHERE vendor_id = $1 AND tenant_id = $2 ORDER BY payment_date DESC', [vendorId, tenantId])).rows;
    ...
    `, [vendorId, tenantId])).rows;
        const reminder = (await pool.query('SELECT enabled, reminder_days, last_reminder_date FROM vendor_reminder_settings WHERE vendor_id = $1 AND tenant_id = $2', [vendorId, tenantId])).rows[0];
...
router.post('/api/vendor-finance/:vendorId/payments', async (req, res) => {
    try {
        ...
        if (batchId) {
            const id = uid('VP');
            await pool.query('INSERT INTO vendor_payments (id, vendor_id, amount, payment_date, payment_method, reference_number, notes, tenant_id, batch_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)', [id, vendorId, parsedAmount, pDate, pMethod, referenceNumber || null, notes || null, tenantId, batchId]);
      ...
      `, [vendorId, tenantId])).rows;
            ...
            for (const b of batches) {
                ...
                const id = uid('VP');
                await pool.query('INSERT INTO vendor_payments (id, vendor_id, amount, payment_date, payment_method, reference_number, notes, tenant_id, batch_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)', [id, vendorId, pay, pDate, pMethod, referenceNumber || null, notes ? `${notes} (batch ${b.batch_id})` : `All-batch payment`, tenantId, b.batch_id]);
                remaining -= pay;
            }
            if (remaining > 0) {
                const id = uid('VP');
                await pool.query('INSERT INTO vendor_payments (id, vendor_id, amount, payment_date, payment_method, reference_number, notes, tenant_id, batch_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)', [id, vendorId, remaining, pDate, pMethod, referenceNumber || null, notes || 'Advance payment', tenantId, null]);
            }
...
router.put('/api/vendor-finance/:vendorId/reminder', async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO vendor_reminder_settings (vendor_id, enabled, reminder_days, tenant_id) VALUES ($1, $2, $3, $4)
       ON CONFLICT (vendor_id, tenant_id) DO UPDATE SET enabled = $2, reminder_days = $3`, [vendorId, enabled ? true : false, reminderDays ?? 7, tenantId]);
        const row = (await pool.query('SELECT * FROM vendor_reminder_settings WHERE vendor_id = $1 AND tenant_id = $2', [vendorId, tenantId])).rows[0];
...
router.post('/api/vendor-finance/bank-statement/preview', async (req, res) => {
    try {
        ...
        const vendors = (await pool.query("SELECT v.id, v.name, v.phone, v.email FROM vendors v WHERE v.tenant_id = $1 AND v.id != 'OWNER'", [tenantId])).rows;
    ...
    `, [tenantId])).rows;
        const paymentRows = (await pool.query('SELECT batch_id, SUM(amount) as paid FROM vendor_payments WHERE tenant_id = $1 GROUP BY batch_id', [tenantId])).rows;
        const paidMap = {};
        for (const p of paymentRows)
            paidMap[p.batch_id] = Number(p.paid);
        ...
        for (const b of batchRows) {
            ...
            if (balance <= 0)
                continue;
            if (!vendorBatches[b.vendor_id])
                vendorBatches[b.vendor_id] = [];
        ...
        for (const vid of Object.keys(vendorBatches))
            vendorBatches[vid].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        ...
        for (let i = 0; i < transactions.length; i++) {
            ...
            if (upiHandle.length === 10)
                allTenDigit.add(upiHandle);
            ...
            for (const v of vendors) {
                ...
                if (!vPhone)
                    continue;
                if (allTenDigit.has(vPhone)) {
                    matchedVendor = v;
                    matchedBy = `phone: ${v.phone}`;
                    break;
                }
            }
            if (matchedVendor) {
...
router.post('/api/vendor-finance/bank-statement/apply', async (req, res) => {
    ...
    try {
        ...
        if (!Array.isArray(payments) || !payments.length)
            return res.status(400).json({ error: 'No payments to apply' });
        const importBatchId = uid('BSI');
        await client.query('BEGIN');
        let count = 0;
        for (const p of payments) {
            if (!p.vendorId || !p.amount || p.amount <= 0)
                continue;
            const id = uid('VP');
            await client.query('INSERT INTO vendor_payments (id, tenant_id, vendor_id, amount, payment_date, payment_method, reference_number, notes, batch_id) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)', [id, tenantId, p.vendorId, p.amount, p.date || new Date().toISOString().slice(0, 10), 'Bank Statement',
