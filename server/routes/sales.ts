...
router.get('/api/sales/validate/:barcode', async (req: AuthRequest, res) => {
  try {
    ...
    const restrictToVendorId = jwtVendorId || (typeof req.query.vendorId === 'string' ? req.query.vendorId : null);
    ...
    const assigned = (
      await pool.query('SELECT vendor_id FROM product_distribution WHERE barcode = $1 AND tenant_id = $2', [
        barcode,
        tenantId,
      ])
    ...
    const exists = (
      await pool.query('SELECT 1 FROM product_inventory WHERE barcode = $1 AND tenant_id = $2', [barcode, tenantId])
    ).rows[0];
...
router.post('/api/sales', blockVendors, async (req: AuthRequest, res) => {
  try {
    ...
    `,
      ...
      | { id: string; product_id: string; vendor_id: string; reward_points_value: number; warranty_months: number }
      | undefined;
    ...
    `,
        ...
        ).rows[0] as { product_id: string; reward_points_value: number; warranty_months: number } | undefined)
      : null;
    ...
    try {
      ...
      const existingCustomer = phoneNorm
        ? ((
            await client.query(
              "SELECT id FROM customers WHERE TRIM(COALESCE(phone, '')) = $1 AND TRIM(COALESCE(name, '')) = $2 AND tenant_id = $3",
              [phoneNorm, String(customerName ?? '').trim(), tenantId],
            )
      ...
      if (existingCustomer) {
        ...
        await client.query(
          'UPDATE customers SET vendor_id = COALESCE(vendor_id, $1) WHERE id = $2 AND tenant_id = $3',
          [vendorId, customerId, tenantId],
        );
      } else {
        customerId = uid('C');
        await client.query(
      ...
      if (dist)
        await client.query("UPDATE product_distribution SET status = 'Sold' WHERE barcode = $1 AND tenant_id = $2", [
          barcode,
          tenantId,
        ]);
      await client.query("UPDATE product_inventory SET status = 'Sold' WHERE barcode = $1 AND tenant_id = $2", [
        barcode,
        tenantId,
      ]);
      await client.query(
    ...
    await logAudit(
      ...
      'sale',
      id,
      `Barcode: ${barcode}, Customer: ${customerName}, Price: ${salePrice ?? 'N/A'}`,
    );
    const row = (await pool.query('SELECT * FROM product_sales WHERE id = $1 AND tenant_id = $2', [id, tenantId]))
...
router.get('/api/sales', async (req, res) => {
  try {
    ...
    const enforcedVendorId =
      jwtUser?.role === 'Vendor' && jwtUser?.vendorId
        ? jwtUser.vendorId
    ...
    if (enforcedVendorId) {
      ...
      params.push(enforcedVendorId);
    }
    where += applyDateFilter(req.query as Record<string, unknown>, 'ps.purchase_date', params);
    const countParams = [...params];
    const total = (
      (await pool.query(`SELECT COUNT(*) as c FROM product_sales ps ${where}`, countParams)).rows[0] as { c: string }
    ).c;
    ...
    res.json({
      data: rows.map(r => ({
        ...
        rewardPointsEarned: r.reward_points_earned,
...
router.get('/api/sales/:id/bill', async (req: AuthRequest, res) => {
  try {
    ...
    const warranty = (
      await pool.query(
        'SELECT activation_date, expiry_date, status FROM warranties WHERE barcode = $1 AND tenant_id = $2 ORDER BY activation_date DESC LIMIT 1',
        [sale.barcode, tenantId],
      )
    ).rows[0] as { activation_date: string; expiry_date: string; status: string } | undefined;
    const company = (
      await pool.query(
    ...
    if (vid && vid !== 'OWNER') {
      ...
      vendorFinance = {
        totalDistributedValue: Number(totalValue.t),
        totalPaid: Number(totalPaid.t),
        balance: Number(totalValue.t) - Number(totalPaid.t),
      };
    }
