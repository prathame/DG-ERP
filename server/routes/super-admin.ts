  ...
  try {
    ...
    await pool.query(
      ...
      [
        ...
        bType,
        accessMode,
        JSON.stringify(mobileFeatures),
        hotelDeployment,
        hotelDbUrlPlain ? encryptSecret(hotelDbUrlPlain) : null,
        result.tenantId,
      ],
    );
    ...
    res.status(201).json({
      ...
      mobileFeatures,
      hotelDeployment,
      hotelDatabaseUrlConfigured: !!hotelDbUrlPlain,
    });
  } catch (err) {
...
router.get('/api/super-admin/version-config', superAdminMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      ...
      desktopAppUrl: cfg['desktop_app_url'] || DEFAULT_DESKTOP_MAC_ARM64_URL,
