...
router.get('/api/price-lists/resolve', async (req, res) => {
    try {
      ...
      ORDER BY
        ...
        min_qty DESC
      LIMIT 1
    `, [tenantId, productId, vendorId || null, qty])).rows[0];
        if (rules) {
            res.json({ price: Number(rules.price), source: 'price_list' });
        }
        else {
            const product = (await pool.query('SELECT price FROM products WHERE id = $1 AND tenant_id = $2', [productId, tenantId])).rows[0];
...
router.post('/api/price-lists', async (req, res) => {
    try {
        ...
        const id = uid('PL');
        await pool.query('INSERT INTO price_lists (id, tenant_id, name, product_id, vendor_id, min_qty, max_qty, price) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)', [id, tenantId, name || 'Custom Price', productId, vendorId || null, Number(minQty) || 1, maxQty ? Number(maxQty) : null, Number(price)]);
