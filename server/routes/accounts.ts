    ...
    if (!type || type === 'all' || type === 'sales') {
      ...
      for (const r of distRows) {
        entries.push({
          date: r.dt as string,
          type: 'Distribution',
          particulars: `Distribution to ${r.vendor_name} (${r.qty} items)`,
          refId: r.ref_id as string,
          debit: Number(r.amount) || 0,
          credit: 0,
        });
      ...
      for (const r of salesRows) {
        entries.push({
          date: r.dt as string,
          type: 'Sale',
          particulars: `Sale to ${r.customer_name} — ${r.product_name}`,
          refId: r.ref_id as string,
          debit: Number(r.amount) || 0,
          credit: 0,
        });
    ...
    if (!type || type === 'all' || type === 'payments') {
      ...
      for (const r of vpRows) {
        entries.push({
          ...
          particulars: `Payment from ${r.vendor_name} (${r.payment_method})`,
          refId: r.ref_id as string,
          debit: Number(r.amount) || 0,
          credit: 0,
        });
...
router.get('/api/accounts/profit-loss', async (req, res) => {
  try {
    ...
    const [cnRes, dnRes] = await Promise.all([
      pool.query(
        "SELECT COALESCE(SUM(subtotal), 0) as t FROM credit_debit_notes WHERE tenant_id = $1 AND note_date >= $2 AND note_date <= $3 AND note_type = 'credit'",
        [tenantId, from, to],
      ),
      pool.query(
        "SELECT COALESCE(SUM(subtotal), 0) as t FROM credit_debit_notes WHERE tenant_id = $1 AND note_date >= $2 AND note_date <= $3 AND note_type = 'debit'",
        [tenantId, from, to],
      ),
    ]);
  ...
  try {
    ...
    const [assetRes, liabilityRes, staffRes, invoiceRes, gstRes] = await Promise.all([
      ...
      pool.query(
        ...
        COALESCE((SELECT SUM(amount) FROM expenses WHERE tenant_id=$1 AND expense_date <= $2),0) as expenses`,
        [tenantId, asOf],
      ),
      pool.query(
        `SELECT
        ...
        COALESCE((SELECT SUM(amount) FROM supplier_payments WHERE tenant_id=$1 AND payment_date <= $2),0) as supplier_paid`,
        [tenantId, asOf],
      ),
      pool.query(
        `SELECT
...
router.get('/api/accounts/cash-flow', async (req, res) => {
  try {
    ...
    const [
      ...
      pool.query(
        "SELECT to_char(payment_date, 'YYYY-MM') as month, SUM(amount) as total FROM invoice_payments WHERE tenant_id = $1 AND payment_date >= $2 AND payment_date <= $3 GROUP BY to_char(payment_date, 'YYYY-MM') ORDER BY month",
        [tenantId, from, to],
      ),
      pool.query(
        "SELECT to_char(payment_date, 'YYYY-MM') as month, SUM(amount) as total FROM supplier_payments WHERE tenant_id = $1 AND payment_date >= $2 AND payment_date <= $3 GROUP BY to_char(payment_date, 'YYYY-MM') ORDER BY month",
        [tenantId, from, to],
      ),
      pool.query(
      ...
      pool.query(
        "SELECT to_char(purchase_date, 'YYYY-MM') as month, SUM(COALESCE(sale_price,0)) as total FROM product_sales WHERE tenant_id = $1 AND purchase_date >= $2 AND purchase_date <= $3 AND vendor_id = 'OWNER' GROUP BY to_char(purchase_date, 'YYYY-MM') ORDER BY month",
        [tenantId, from, to],
      ),
    ]);
    ...
    const months = new Set([
      ...
      ...monthlyInv.map(r => r.month),
      ...monthlyOwner.map(r => r.month),
      ...monthlyOut.map(r => r.month),
      ...monthlyStaff.map(r => r.month),
      ...monthlyExp.map(r => r.month),
    ]);
    const inMap: Record<string, number> = {};
    for (const r of monthlyIn) inMap[r.month] = Number(r.total) || 0;
    const invMap: Record<string, number> = {};
    for (const r of monthlyInv) invMap[r.month] = Number(r.total) || 0;
    const ownerMap: Record<string, number> = {};
    for (const r of monthlyOwner) ownerMap[r.month] = Number(r.total) || 0;
    const outMap: Record<string, number> = {};
    for (const r of monthlyOut) outMap[r.month] = Number(r.total) || 0;
    const staffMap: Record<string, number> = {};
    for (const r of monthlyStaff) staffMap[r.month] = Number(r.total) || 0;
    const expMap: Record<string, number> = {};
    for (const r of monthlyExp) expMap[r.month] = Number(r.total) || 0;
...
router.post('/api/accounts/notes', blockVendors, async (req: AuthRequest, res) => {
  try {
    ...
    const count = (
      await pool.query('SELECT COUNT(*) as c FROM credit_debit_notes WHERE tenant_id = $1 AND note_type = $2', [
        tenantId,
        noteType,
      ])
    ).rows[0] as { c: number };
    ...
    await logAudit(
      pool,
      tenantId,
      `${noteType === 'credit' ? 'Credit' : 'Debit'} Note Created`,
      'note',
      id,
...
router.get('/api/accounts/day-book', async (req, res) => {
  try {
    ...
    const [sales, invoices, distributions, purchases, vendorPayments, supplierPayments, staffPays, expenseRows] =
      await Promise.all([
        pool.query(
          `SELECT ps.id, ps.purchase_date as date, ps.customer_name, COALESCE(ps.sale_price, p.price) as amount, p.name as product_name, 'Sale' as type
        FROM product_sales ps JOIN products p ON ps.product_id = p.id AND p.tenant_id = $1
        WHERE ps.tenant_id = $1 AND ps.purchase_date = $2 AND ps.vendor_id = 'OWNER'`,
          [tenantId, date],
        ),
        pool.query(
        ...
        pool.query(
          `SELECT pd.batch_id as id, pd.distribution_date as date, v.name as party_name,
        ...
        pool.query(
          `SELECT pp.batch_id as id, pp.purchase_date as date, s.name as party_name,
        ...
        pool.query(
          `SELECT vp.id, vp.payment_date as date, v.name as party_name, vp.amount, vp.payment_method, 'Payment Received' as type
        FROM vendor_payments vp JOIN vendors v ON vp.vendor_id = v.id AND v.tenant_id = $1
        WHERE vp.tenant_id = $1 AND vp.payment_date = $2`,
          [tenantId, date],
        ),
        pool.query(
          `SELECT sp.id, sp.payment_date as date, s.name as party_name, sp.amount, sp.payment_method, 'Payment Made' as type
        FROM supplier_payments sp JOIN suppliers s ON sp.supplier_id = s.id AND s.tenant_id = $1
    ...
    for (const r of sales.rows as Record<string, unknown>[]) {
      entries.push({
        ...
        type: 'Sale',
        party: (r.customer_name as string) || 'Walk-in',
    ...
    for (const r of staffPays.rows as Record<string, unknown>[]) {
      ...
      entries.push({
        id: r.id as string,
        date: r.date as string,
        type: typeLabels[pType] || 'Staff Payment',
        party: r.staff_name as string,
    ...
    for (const r of expenseRows.rows as Record<string, unknown>[]) {
      entries.push({
        ...
        party: (r.description as string) || (r.category as string),
        debit: 0,
        credit: Number(r.amount) || 0,
        method: r.payment_method as string,
      });
...
router.get('/api/gstr3b/compute', async (req, res) => {
  try {
    ...
    const sellerGstin =
      (
        (await pool.query('SELECT gst_number FROM tenants WHERE id = $1', [tenantId])).rows[0] as
          { gst_number?: string } | undefined
      )?.gst_number ||
      (
        (
          await pool.query(
            "SELECT gst_number FROM users WHERE tenant_id = $1 AND role IN ('Admin','Super Admin') LIMIT 1",
            [tenantId],
          )
    ...
    for (const p of purchases) {
      ...
      if (existing) {
        ...
        bookMap.set(key, {
          ...
          totalBilled: Number(p.billed_price) || 0,
          totalCost: Number(p.cost_price) || 0,
          count: 1,
        });
      }
    }
    ...
    for (const supplier of b2b as { ctin?: string; trdnm?: string; inv?: Record<string, unknown>[] }[]) {
      ...
      for (const inv of invoices) {
        ...
        const items = Array.isArray(inv.items)
          ? (inv.items as { txval?: number; igst?: number; cgst?: number; sgst?: number }[])
          : [];
        ...
        if (book) {
          ...
          rows.push({
            ...
            twoBVal,
            bookVal: book.totalBilled,
            diff: Math.round((twoBVal - book.totalBilled) * 100) / 100,
            itcAvailable,
          });
        } else {
          rows.push({
