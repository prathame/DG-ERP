...
router.post('/api/banks/batch', async (req, res) => {
    ...
    try {
        ...
        for (const r of items) {
            ...
            const id = uid('B');
            await client.query('INSERT INTO banks (id, tenant_id, name, account_number, bank_name, branch, ifsc_code) VALUES ($1,$2,$3,$4,$5,$6,$7)', [id, tenantId, name, acNo || null, r.bankName || null, r.branch || null, r.ifscCode || null]);
            count++;
        }
        ...
        res.status(500).json({ error: e.message || 'Import failed — no banks were added' });
    }
    finally {
        client.release();
    }
...
router.post('/api/banks', async (req, res) => {
    try {
        ...
        if (accountNumber) {
            ...
            if (dup)
                return res.status(400).json({ error: `Account number "${accountNumber}" already exists` });
        }
        const id = uid('B');
        await pool.query('INSERT INTO banks (id, tenant_id, name, account_number, bank_name, branch, ifsc_code) VALUES ($1, $2, $3, $4, $5, $6, $7)', [id, tenantId, name.trim(), accountNumber, bankName, branch, ifscCode]);
...
router.put('/api/banks/:id', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const { name, accountNumber, bankName, branch, ifscCode } = req.body;
        const result = await pool.query('UPDATE banks SET name=COALESCE($1,name), account_number=COALESCE($2,account_number), bank_name=COALESCE($3,bank_name), branch=COALESCE($4,branch), ifsc_code=COALESCE($5,ifsc_code) WHERE id=$6 AND tenant_id=$7', [name, accountNumber, bankName, branch, ifscCode, id, tenantId]);
