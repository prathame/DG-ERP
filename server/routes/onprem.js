...
router.post('/api/onprem/activate', async (req, res) => {
    try {
        ...
        const lic = (await pool.query('SELECT * FROM onprem_licenses WHERE license_key = $1', [licenseKey])).rows[0];
        if (!lic)
            return res.status(404).json({ error: 'Invalid license key' });
        if (lic.status !== 'active')
            return res.status(403).json({ error: `License ${lic.status}` });
        if (lic.valid_until && new Date(lic.valid_until) < new Date()) {
        ...
        await pool.query(`UPDATE onprem_licenses SET machine_id=$1, machine_os=$2, app_version=$3, last_seen=NOW() WHERE license_key=$4`, [machineId, osInfo || null, appVersion || null, licenseKey]);
        res.json({
            valid: true,
        ...
        console.error('💥 /api/onprem/activate failed:', err.message);
...
router.post('/api/onprem/heartbeat', async (req, res) => {
    try {
        ...
        const lic = (await pool.query('SELECT * FROM onprem_licenses WHERE license_key = $1', [licenseKey])).rows[0];
        if (!lic)
            return res.json({ licenseValid: false, message: 'Unknown license' });
        ...
        if (isValid && isMachineMatch) {
            await pool.query(`UPDATE onprem_licenses SET last_seen=NOW(), app_version=$1, active_users=$2, disk_mb=$3 WHERE license_key=$4`, [version || null, activeUsers || 0, diskMB || 0, licenseKey]);
        }
        ...
        const daysLeft = lic.valid_until
            ? Math.ceil((new Date(lic.valid_until).getTime() - Date.now()) / 86400000)
            : null;
        res.json({
        ...
        console.error('💥 /api/onprem/heartbeat failed:', err.message);
        res.status(500).json({ error: 'Internal server error' });
    }
});
...
router.post('/api/onprem/deactivate', async (req, res) => {
    try {
        ...
        const lic = (await pool.query('SELECT * FROM onprem_licenses WHERE license_key = $1 AND machine_id = $2', [licenseKey, machineId])).rows[0];
        if (!lic)
            return res.status(404).json({ error: 'License not found for this machine' });
...
router.post('/api/onprem/provision', async (req, res) => {
    try {
        ...
        const planRow = (await pool.query("SELECT id FROM plans LIMIT 1")).rows[0];
        ...
        await pool.query('UPDATE tenants SET business_type=$1 WHERE id=$2', [businessType || 'manufacturer', result.tenantId]);
        res.json({ ok: true, tenantId: result.tenantId, slug: result.slug });
    }
    catch (err) {
        console.error('💥 /api/onprem/provision failed:', err.message);
...
router.get('/api/super-admin/onprem', superAdminMiddleware, async (req, res) => {
    try {
        const rows = (await pool.query(`SELECT *, (valid_until IS NULL OR valid_until >= CURRENT_DATE) AS is_valid_date FROM onprem_licenses ORDER BY created_at DESC`)).rows;
        const now = Date.now();
        res.json(rows.map((r) => ({
            id: r.id,
            ...
            diskMB: r.disk_mb,
            settings: r.settings,
            isOnline: r.last_seen && (now - new Date(r.last_seen).getTime()) < 70 * 60 * 1000, // online if seen < 70 min ago
...
router.post('/api/super-admin/onprem', superAdminMiddleware, async (req, res) => {
    try {
        ...
        await pool.query(`INSERT INTO onprem_licenses (id, license_key, company_name, business_type, admin_email, max_users, valid_until, settings, created_by)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [id, licenseKey, companyName, businessType || 'manufacturer', adminEmail || null,
...
router.put('/api/super-admin/onprem/:id', superAdminMiddleware, async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE onprem_licenses SET ${updates.join(',')} WHERE id=$${idx} RETURNING *`, params);
        if (!result.rows[0])
