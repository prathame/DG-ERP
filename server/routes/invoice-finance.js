...
router.post('/api/invoice-finance/payments', async (req, res) => {
    ...
    try {
        ...
        const inv = (await pool.query('SELECT id, grand_total, customer_name FROM standalone_invoices WHERE id = $1 AND tenant_id = $2 AND status != $3', [invoiceId, tenantId, 'cancelled'])).rows[0];
        if (!inv)
            return res.status(404).json({ error: 'Invoice not found' });
        const id = uid('IP');
        const pDate = paymentDate || new Date().toISOString().slice(0, 10);
        await client.query('BEGIN');
        await client.query('INSERT INTO invoice_payments (id, tenant_id, invoice_id, amount, payment_date, payment_method, reference_number, notes) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)', [id, tenantId, invoiceId, Number(amount), pDate, paymentMethod || 'Cash', referenceNumber || null, notes || null]);
        // Auto-mark invoice as paid if fully paid
        const totalPaid = Number((await client.query('SELECT COALESCE(SUM(amount),0) as t FROM invoice_payments WHERE invoice_id = $1 AND tenant_id = $2', [invoiceId, tenantId])).rows[0].t);
...
router.delete('/api/invoice-finance/payments/:id', async (req, res) => {
    ...
    try {
        ...
        if (!tenantId)
            return res.status(401).json({ error: 'Tenant ID required' });
        const payment = (await pool.query('SELECT invoice_id, amount FROM invoice_payments WHERE id = $1 AND tenant_id = $2', [req.params.id, tenantId])).rows[0];
        if (!payment)
        ...
        const remaining = Number((await client.query('SELECT COALESCE(SUM(amount),0) as t FROM invoice_payments WHERE invoice_id = $1 AND tenant_id = $2', [payment.invoice_id, tenantId])).rows[0].t);
        const inv = (await client.query('SELECT grand_total FROM standalone_invoices WHERE id = $1 AND tenant_id = $2', [payment.invoice_id, tenantId])).rows[0];
