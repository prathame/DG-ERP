...
router.get('/api/reports/outstanding', async (req, res) => {
    ...
    `, [tenantId])).rows;
      ...
      `, [tenantId, v.id])).rows;
            const batchPayments = {};
            const payRows = (await pool.query('SELECT batch_id, SUM(amount) as paid FROM vendor_payments WHERE vendor_id = $1 AND tenant_id = $2 AND batch_id IS NOT NULL GROUP BY batch_id', [v.id, tenantId])).rows;
            for (const pr of payRows)
                batchPayments[pr.batch_id] = Number(pr.paid);
...
router.get('/api/reports/gst-summary', async (req, res) => {
    ...
    `, [tenantId, startDate, endDate])).rows;
        const b2b = {};
        ...
        for (const r of distRows) {
            ...
            const halfGst = Math.round(gstAmt / 2);
            const gstin = r.vendor_gstin || '';
            const hsn = r.hsn_code || 'N/A';
            if (gstin) {
                if (!b2b[gstin])
                    b2b[gstin] = { vendorName: r.vendor_name, gstin, taxable: 0, cgst: 0, sgst: 0, total: 0, invoiceCount: 0 };
            ...
            if (!hsnMap[hsn])
                hsnMap[hsn] = { hsn, description: r.product_name, qty: 0, taxable: 0, cgst: 0, sgst: 0, total: 0 };
            hsnMap[hsn].qty++;
...
router.get('/api/reports/gstr1', async (req, res) => {
    try {
        ...
        const tenant = (await pool.query('SELECT company_name, gst_number FROM tenants WHERE id = $1', [tenantId])).rows[0];
        const user = (await pool.query("SELECT gst_number FROM users WHERE tenant_id = $1 AND role = 'Admin' LIMIT 1", [tenantId])).rows[0];
    ...
    `, [tenantId, startDate, endDate])).rows;
        ...
        for (const r of distRows) {
            ...
            if (hsn) {
                if (!hsnMap[hsn])
                    hsnMap[hsn] = { hsn, desc: r.product_name, uqc: 'PCS', qty: 0, taxable: 0, igst: 0, cgst: 0, sgst: 0, rate: gstRate };
                hsnMap[hsn].qty++;
                hsnMap[hsn].taxable += net;
                hsnMap[hsn].cgst += halfGst;
                hsnMap[hsn].sgst += gstAmt - halfGst;
            ...
            if (gstin && gstin.length >= 15) {
                ...
                if (!invoiceMap[key])
                    invoiceMap[key] = { batchId, date: r.distribution_date, vendorName: r.vendor_name, gstin, items: [] };
                ...
                if (existing) {
                    ...
                    invoiceMap[key].items.push({ hsn, name: r.product_name, qty: 1, rate: gstRate, taxable: net, cgst: halfGst, sgst: gstAmt - halfGst, total: billed });
                }
            }
            else {
                // B2C
                b2cItems.push({ hsn, name: r.product_name, qty: 1, taxable: net, cgst: halfGst, sgst: gstAmt - halfGst, total: billed });
            }
        }
        ...
        for (const inv of Object.values(invoiceMap)) {
            if (!b2bByGstin[inv.gstin])
                b2bByGstin[inv.gstin] = { ctin: inv.gstin, cfs: 'Y', inv: [] };
        ...
        for (const item of b2cItems) {
            ...
            const key = `${rate}`;
            if (!b2csGrouped[key])
                b2csGrouped[key] = { rt: rate, txval: 0, camt: 0, samt: 0, iamt: 0 };
