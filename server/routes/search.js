...
router.get('/api/search', async (req, res) => {
    try {
        ...
        if (!q || q.length < 1)
            return res.json({ products: [], customers: [], vendors: [], warranties: [], sales: [] });
        const like = `%${q}%`;
        const limit = 6;
    ...
    `, [like, tenantId, limit])).rows;
        res.json({
            products: products.map((p) => ({ id: p.id, name: p.name, price: p.price, stock: p.stock, type: 'product' })),
            customers: customers.map((c) => ({ id: c.id, name: c.name, phone: c.phone ?? '', email: c.email ?? '', type: 'customer' })),
            vendors: vendors.map((v) => ({ id: v.id, name: v.name, contact: v.contact_person ?? '', phone: v.phone ?? '', type: 'vendor' })),
            barcodes: barcodeResults.map((b) => ({ barcode: b.barcode, productName: b.product_name, productId: b.product_id, status: b.status, type: 'barcode' })),
            challans: challans.map((c) => ({ batchId: c.batch_id, vendorName: c.vendor_name, date: c.distribution_date, units: Number(c.total_units), type: 'challan' })),
            staff: staff.map((s) => ({ name: s.staff_name, totalPaid: Number(s.total_paid), payments: Number(s.payments), lastPayment: s.last_payment, type: 'staff' })),
        });
    }
    catch (err) {
