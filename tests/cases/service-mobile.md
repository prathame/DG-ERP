...
| 5d4 | Invoice WhatsApp PDF (safe) | Invoices / Client hub / Invoice Finance → WhatsApp | Toast "Preparing PDF…". App stays responsive. Prefer PDF Share (file-only); if slow/fail → text summary + toast explaining PDF failed. Print still full Tax Invoice HTML. Bug report includes WhatsApp share breadcrumbs/logs |
