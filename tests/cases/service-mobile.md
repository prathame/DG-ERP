...
| 5d4 | Invoice WhatsApp PDF (safe) | Settings → Bill Customization Save, then Invoices / Client / Invoice Finance → WhatsApp | Save updates brand template. WhatsApp toast "Preparing PDF…"; Cap uses shared jsPDF builder (template + invoice fields only) — print-like, not html2pdf. Multi-page PDFs keep a footer reserve clear of item rows (wrap-aware); page 2+ repeats company/logo strip + “continued” + page #; bank/totals/sign in the reserve on last page only. Invoice/Quote Save bakes PDF under `Documents/Dhandho/invoices/` (async; bake fail does not fail Save); WhatsApp reuses when contentKey matches, else builds on demand. File-only Share preferred; timeout/fail → wa.me text (not Gmail/email). Print still full Tax Invoice HTML. Bug report has WhatsApp breadcrumbs. |
