...
| 5d4 | Invoice WhatsApp (no WebView freeze) | Invoices / Client hub / Invoice Finance → WhatsApp | App stays responsive. Share sheet / WhatsApp opens with **text summary only** (no PDF gen on tap). Toast mentions Print → Save as PDF for a file. Print still full Tax Invoice HTML |
...
| 5m4 | Quote → Convert to Invoice | Masters client → Quote Accepted → **Convert to Invoice** | Invoice appears on **Invoice** tab (`sent`); same client hub under Masters → Client shows it; Analytics **This Month** → Invoice Revenue + Unpaid Invoices include the amount (Received stays 0 until payment) |
