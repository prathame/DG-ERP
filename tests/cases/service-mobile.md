...
| 5d4 | Invoice WhatsApp PDF (safe) | Invoices / Client hub / Invoice Finance → WhatsApp | Toast "Preparing PDF…". App stays responsive. Cap PDF is light jsPDF using **saved Bill Customization** as template (logo, signature, bank, color, terms, footer, tagline) + invoice fields — print-like layout, not html2pdf. Prefer PDF Share (file-only); if slow/fail → text summary + toast. Print still full Tax Invoice HTML. Bug report includes WhatsApp share breadcrumbs/logs |
