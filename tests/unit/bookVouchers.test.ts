...
describe('bookVouchers', () => {
  ...
  it('dual-writes debit_note items to ops Sold stock when products resolve', async () => {
    await cleanupTestData(TENANT);
    await pool.query('ALTER TABLE suppliers ADD COLUMN IF NOT EXISTS external_ref TEXT');
    await pool.query('ALTER TABLE products ADD COLUMN IF NOT EXISTS external_ref TEXT');
    const { party } = await seedLedgers();
    ...
    const supplierId = uid('SU');
    await pool.query(`INSERT INTO suppliers (id, tenant_id, name) VALUES ($1,$2,'Seed Supp DN')`, [supplierId, TENANT]);
    const productId = uid('PR');
    await pool.query(
      `INSERT INTO products (id, tenant_id, name, price, stock, external_ref) VALUES ($1,$2,'Bolt DN',10,0,'X-BOLT-DN')`,
      [productId, TENANT],
    );
    ...
    try {
      ...
      expect(created.ops.dualWrite).toBe('debit_note');
      expect(created.ops.stockUnits).toBe(2);
