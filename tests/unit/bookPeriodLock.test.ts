...
describe('bookPeriodLock', () => {
  ...
  it('rejects any locked date in a multi-document operation', async () => {
    const db = { query: async () => ({ rows: [{ lock_date: '2026-03-31' }] }) } as unknown as Pool;
    await expect(assertBooksDatesUnlocked(db, 'T-LOCK', ['2026-04-01', '2026-03-31'])).rejects.toMatchObject({
      status: 409,
      message: 'Books are closed through 2026-03-31. Cannot change vouchers dated on or before that day.',
    });
  });
});
