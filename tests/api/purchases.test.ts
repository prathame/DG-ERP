...
describe('Purchases & Suppliers', () => {
  ...
  describe('Suppliers CRUD', () => {
    it('creates supplier', async () => {
      ...
      const { rows } = await pool.query('SELECT * FROM suppliers WHERE id = $1 AND tenant_id = $2', [
        'S-1',
        TEST_TENANT,
      ]);
    ...
    it('lists suppliers ordered by name + search', async () => {
      ...
      const { rows } = await pool.query('SELECT name FROM suppliers WHERE tenant_id = $1 ORDER BY name', [TEST_TENANT]);
    ...
    it('updates supplier fields', async () => {
      await pool.query(`UPDATE suppliers SET contact_person = $1, address = $2 WHERE id = $3 AND tenant_id = $4`, [
        'Kiran Updated',
        'Jamnagar',
        'S-1',
        TEST_TENANT,
    ...
    it('is tenant-isolated', async () => {
      ...
      const { rows } = await pool.query('SELECT * FROM suppliers WHERE tenant_id = $1 AND name = $2', [
        TEST_TENANT,
        'Other Supplier',
      ]);
  ...
  describe('Purchase batches', () => {
    it('creates purchase batch rows with cost / billed / GST', async () => {
      ...
      const billedPricePerUnit = gstApplied
        ? Math.round(((costPricePerUnit * (100 + gstRate)) / 100) * 100) / 100
        : costPricePerUnit;
      expect(costPricePerUnit).toBe(90);
      expect(billedPricePerUnit).toBe(106.2); // 90 * 1.18 = 106.20 paisa
      ...
      expect(Number(rows[0].billed)).toBeCloseTo(318.6, 2);
    });
    ...
    it('lists batches grouped with totals', async () => {
      ...
      expect(Number(b.total_billed)).toBeCloseTo(318.6, 2);
    });
  ...
  describe('Supplier finance', () => {
    it('summary computes purchased / paid / balance', async () => {
      ...
      expect(Number(steel.total_purchased_value)).toBeCloseTo(318.6, 2);
      expect(Number(steel.total_paid)).toBe(0);
      expect(Number(steel.units_purchased)).toBe(3);
      expect(Number(steel.total_purchased_value) - Number(steel.total_paid)).toBeCloseTo(318.6, 2);
    });
    ...
    it('detects batch overpay', async () => {
      ...
      expect(remaining).toBeCloseTo(218.6, 2);
    ...
    it('404 when supplier missing', async () => {
      const supplier = (
        await pool.query('SELECT * FROM suppliers WHERE id = $1 AND tenant_id = $2', ['S-MISSING', TEST_TENANT])
      ).rows[0];
