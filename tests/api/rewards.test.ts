...
describe('Rewards', () => {
  ...
  describe('GET /api/rewards', () => {
    it('lists all rewards for tenant', async () => {
      const { rows } = await pool.query('SELECT * FROM rewards WHERE tenant_id = $1 ORDER BY date DESC', [TEST_TENANT]);
    ...
    it('filters by vendorId', async () => {
      ...
      const { rows } = await pool.query("SELECT * FROM rewards WHERE tenant_id = $1 AND vendor_id = 'V-RWD-1'", [
        TEST_TENANT,
      ]);
    ...
    it('does not leak other tenant rewards', async () => {
      ...
      const { rows } = await pool.query('SELECT * FROM rewards WHERE tenant_id = $1', [TEST_TENANT]);
  ...
  describe('POST /api/rewards — Earned', () => {
    it('inserts an Earned reward', async () => {
      ...
      const { rows } = await pool.query('SELECT * FROM rewards WHERE id = $1 AND tenant_id = $2', [id, TEST_TENANT]);
    ...
    it('increments vendor total_reward_points when vendorId provided on Earned', async () => {
      const before = (
        await pool.query('SELECT total_reward_points FROM vendors WHERE id = $1 AND tenant_id = $2', [
          'V-RWD-1',
          TEST_TENANT,
        ])
  ...
  describe('POST /api/rewards — Redeemed', () => {
    beforeAll(async () => {
      ...
      await pool.query('UPDATE vendors SET total_reward_points = 500 WHERE id = $1 AND tenant_id = $2', [
        'V-RWD-1',
        TEST_TENANT,
      ]);
    });
    ...
    it('rejects when vendor balance below minBalance', async () => {
      await pool.query('UPDATE vendors SET total_reward_points = 10 WHERE id = $1 AND tenant_id = $2', [
        'V-RWD-2',
        TEST_TENANT,
      ]);
      const settings = (
    ...
    it('vendor redemption decrements vendor total_reward_points', async () => {
      ...
      const reward = (await pool.query('SELECT * FROM rewards WHERE id = $1 AND tenant_id = $2', [id, TEST_TENANT]))
        .rows[0];
    ...
    it('non-vendor redemption (general balance) succeeds with enough balance', async () => {
      ...
      const row = (await pool.query('SELECT * FROM rewards WHERE id = $1 AND tenant_id = $2', [id, TEST_TENANT]))
        .rows[0];
  ...
  describe('PUT /api/rewards/:id', () => {
    ...
    beforeAll(async () => {
      ...
      await pool.query('UPDATE vendors SET total_reward_points = 300 WHERE id = $1 AND tenant_id = $2', [
        'V-RWD-1',
        TEST_TENANT,
      ]);
    });
    ...
    it('updates points and description', async () => {
      await pool.query(`UPDATE rewards SET points = 150, description = 'updated' WHERE id = $1 AND tenant_id = $2`, [
        rewardId,
        TEST_TENANT,
      ]);
      const row = (
        await pool.query('SELECT points, description FROM rewards WHERE id = $1 AND tenant_id = $2', [
          rewardId,
          TEST_TENANT,
        ])
      ).rows[0];
    ...
    it('returns 404 when reward not found in tenant', async () => {
      const result = await pool.query('UPDATE rewards SET points = 999 WHERE id = $1 AND tenant_id = $2', [
        'RW-NONEXISTENT',
        TEST_TENANT,
      ]);
  ...
  describe('DELETE /api/rewards/:id', () => {
    it('deletes a reward and reverses vendor Earned counter', async () => {
      ...
      await pool.query('UPDATE vendors SET total_reward_points = 200 WHERE id = $1 AND tenant_id = $2', [
        'V-RWD-1',
        TEST_TENANT,
      ]);

      const existing = (await pool.query('SELECT * FROM rewards WHERE id = $1 AND tenant_id = $2', [id, TEST_TENANT]))
        .rows[0];
      ...
      const gone = (await pool.query('SELECT * FROM rewards WHERE id = $1 AND tenant_id = $2', [id, TEST_TENANT]))
        .rows[0];
    ...
    it('returns 404 when reward not found', async () => {
      const result = await pool.query('DELETE FROM rewards WHERE id = $1 AND tenant_id = $2', [
        'RW-NONEXISTENT',
        TEST_TENANT,
      ]);
      expect(result.rowCount).toBe(0);
  ...
  describe('GET /api/reward-rules', () => {
    it('returns empty array when no rules', async () => {
      const { rows } = await pool.query('SELECT * FROM reward_rules WHERE tenant_id = $1', [OTHER_TENANT]);
  ...
  describe('POST /api/reward-rules', () => {
    it('inserts a rule with all fields', async () => {
      ...
      const row = (await pool.query('SELECT * FROM reward_rules WHERE id = $1 AND tenant_id = $2', [id, TEST_TENANT]))
        .rows[0];
    ...
    it('inserts a rule with nullable categoryId', async () => {
      ...
      const row = (
        await pool.query('SELECT category_id FROM reward_rules WHERE id = $1 AND tenant_id = $2', [id, TEST_TENANT])
      ).rows[0];
  ...
  describe('PUT /api/reward-rules/:id', () => {
    it('updates reward rule fields', async () => {
      ...
      const row = (
        await pool.query('SELECT reward_points, description FROM reward_rules WHERE id = $1 AND tenant_id = $2', [
          'RR-1',
          TEST_TENANT,
        ])
      ).rows[0];
    ...
    it('returns 404 for nonexistent rule', async () => {
      const result = await pool.query('UPDATE reward_rules SET reward_points = 1 WHERE id = $1 AND tenant_id = $2', [
        'RR-NONEXISTENT',
        TEST_TENANT,
      ]);
      expect(result.rowCount).toBe(0);
    ...
    it('does not update rule from other tenant', async () => {
      const result = await pool.query('UPDATE reward_rules SET reward_points = 999 WHERE id = $1 AND tenant_id = $2', [
        'RR-1',
        OTHER_TENANT,
      ]);
      expect(result.rowCount).toBe(0);
  ...
  describe('DELETE /api/reward-rules/:id', () => {
    it('deletes an existing rule', async () => {
      ...
      const result = await pool.query('DELETE FROM reward_rules WHERE id = $1 AND tenant_id = $2', [id, TEST_TENANT]);
      expect(result.rowCount).toBe(1);
      const gone = (await pool.query('SELECT * FROM reward_rules WHERE id = $1', [id])).rows[0];
      expect(gone).toBeUndefined();
    });

    it('returns 404 for nonexistent rule', async () => {
      const result = await pool.query('DELETE FROM reward_rules WHERE id = $1 AND tenant_id = $2', [
        'RR-NONEXISTENT',
        TEST_TENANT,
      ]);
      expect(result.rowCount).toBe(0);
  ...
  describe('Balance edge cases', () => {
    ...
    it('redemption_settings row missing → fallback 100/50', () => {
      const row = undefined as { min_balance: number; min_points: number } | undefined;
