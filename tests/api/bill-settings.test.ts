...
describe('Bill Settings', () => {
  ...
  it('should create bill settings', async () => {
    ...
    const { rows } = await pool.query('SELECT * FROM bill_settings WHERE tenant_id = $1', [TEST_TENANT]);
  ...
  it('should update logo with base64 data', async () => {
    ...
    await pool.query('UPDATE bill_settings SET logo_base64 = $1 WHERE tenant_id = $2', [fakeBase64, TEST_TENANT]);
    const { rows } = await pool.query('SELECT logo_base64 FROM bill_settings WHERE tenant_id = $1', [TEST_TENANT]);
  ...
  it('should update primary color with hex value', async () => {
    ...
    await pool.query('UPDATE bill_settings SET primary_color = $1 WHERE tenant_id = $2', [newColor, TEST_TENANT]);
    const { rows } = await pool.query('SELECT primary_color FROM bill_settings WHERE tenant_id = $1', [TEST_TENANT]);
  ...
  it('should update terms and conditions', async () => {
    ...
    await pool.query('UPDATE bill_settings SET terms_and_conditions = $1 WHERE tenant_id = $2', [terms, TEST_TENANT]);
    const { rows } = await pool.query('SELECT terms_and_conditions FROM bill_settings WHERE tenant_id = $1', [
      TEST_TENANT,
    ]);
  ...
  it('should toggle show/hide settings', async () => {
    await pool.query(
      'UPDATE bill_settings SET show_rewards = false, show_barcode = false, show_warranty = true, show_hsn_sac = false WHERE tenant_id = $1',
      [TEST_TENANT],
    );
    const { rows } = await pool.query(
      'SELECT show_rewards, show_barcode, show_warranty, show_hsn_sac FROM bill_settings WHERE tenant_id = $1',
      [TEST_TENANT],
    );
    expect(rows[0].show_rewards).toBe(false);
    expect(rows[0].show_barcode).toBe(false);
    expect(rows[0].show_warranty).toBe(true);
    expect(rows[0].show_hsn_sac).toBe(false);

    // Toggle back
    await pool.query(
      'UPDATE bill_settings SET show_rewards = true, show_barcode = true, show_hsn_sac = true WHERE tenant_id = $1',
      [TEST_TENANT],
    );
    const updated = (
      await pool.query('SELECT show_rewards, show_barcode, show_hsn_sac FROM bill_settings WHERE tenant_id = $1', [
        TEST_TENANT,
      ])
    ).rows[0];
    expect(updated.show_rewards).toBe(true);
    expect(updated.show_barcode).toBe(true);
    expect(updated.show_hsn_sac).toBe(true);
  });
