...
export function CsvImport({
  ...
  const applyParsed = (parsed: { headers: string[]; rows: Record<string, string>[] }) => {
    if (parsed.rows.length === 0) {
      setError('No data rows found in file');
      return;
    }
    const requiredCols = columns.filter(c => c.required).map(c => c.key);
    const missing = requiredCols.filter(k => !hasHeader(parsed.headers, k));
    if (missing.length > 0) {
      setError(`Missing required columns: ${missing.join(', ')}`);
      return;
    }
    if (requireAnyOf?.length) {
      for (const group of requireAnyOf) {
        if (!group.some(k => hasHeader(parsed.headers, k))) {
          setError(`Missing one of required columns: ${group.join(' or ')}`);
          return;
        }
      }
    }
    ...
    try {
      ...
      if (nameMatch && !rowMatch) {
        const idx = rows.findIndex(r =>
          Object.values(r).some(v => String(v).toLowerCase() === nameMatch[1].toLowerCase()),
        );
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h3 className="font-bold text-lg flex items-center gap-2">
            <Upload size={20} className="text-brand" /> Import {itemLabel} from CSV / Excel
          </h3>
            ...
            <div className="space-y-4">
                ...
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-rose-600 mt-1">
                      Fix the error below and try again. All rows must be valid for the import to succeed.
                    </p>
                  </div>
                </div>
              ) : (
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-bold text-emerald-800">
                      {result.success} {itemLabel} processed successfully
                    </p>
                    {(result as Record<string, unknown>).created != null && (
                      <div className="text-sm text-emerald-700 mt-1 space-y-0.5">
                        {Number((result as Record<string, unknown>).created) > 0 && (
                          <p>
                            + {(result as Record<string, unknown>).created} new {itemLabel} created
                          </p>
                        )}
                        {Number((result as Record<string, unknown>).stockAdded) > 0 && (
                          <p>
                            + {(result as Record<string, unknown>).stockAdded} existing {itemLabel} — stock added
                          </p>
                        )}
                      </div>
                    )}
                    {Array.isArray((result as Record<string, unknown>).details) && (
                      <div className="mt-2 max-h-32 overflow-y-auto space-y-0.5">
                        {(
                          (result as Record<string, unknown>).details as {
                            name: string;
                            action: string;
                            quantity: number;
                          }[]
              ...
              {result.errors.length > 0 && (
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-4 max-h-40 overflow-y-auto">
                  ...
                  {result.errors.map((err, i) => (
                    <p key={i} className="text-sm text-rose-600 flex items-start gap-1.5 mb-1">
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="bg-gray-50 border-b border-gray-200">
                          ...
                          {headers.map(h => (
                            <th key={h} className="px-3 py-2 text-left font-bold text-gray-500">
                              {h}
                            </th>
                          ))}
                        </tr>
                        ...
                        {rows.map((row, i) => (
                          <tr
                            ...
                            className={cn(
                              ...
                              errorRow === i ? 'bg-rose-50 border-rose-200' : '',
                            )}
                          >
                            <td
                              className={cn('px-3 py-2', errorRow === i ? 'text-rose-600 font-bold' : 'text-gray-400')}
                            >
                              {i + 2}
                              {errorRow === i && ' ⚠️'}
                            </td>
                            {headers.map(h => (
                              <td
                                key={h}
                                className={cn('px-3 py-2', errorRow === i ? 'text-rose-700 font-medium' : '')}
                              >
                                {row[h] || '-'}
                              </td>
                            ))}
              ...
              <button
                ...
                className={cn(
                  ...
                  failed ? 'bg-gray-100 text-gray-700 hover:bg-gray-200' : 'bg-brand text-white',
                )}
              >
            ...
            <>
              <div className="flex gap-3">
                ...
                <label className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 border-2 border-dashed border-gray-300 rounded-xl text-sm font-medium cursor-pointer hover:border-brand hover:bg-brand/5 transition-colors">
                  <Upload size={16} /> {rows.length > 0 ? `${rows.length} rows loaded` : 'Choose CSV / Excel file'}
                  ...
                  {columns.map(c => (
                    <span
                      ...
                      className={cn(
                        ...
                        c.required ? 'bg-brand/10 text-brand' : 'bg-gray-200 text-gray-500',
                      )}
                    >
                      {c.key} {c.required ? '*' : ''}
                    </span>
                  ...
                  return (
                    <>
                      {existingNames && stockCount > 0 && (
                        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 flex items-center gap-2 text-sm">
                          ...
                          <span className="text-blue-800">
                            <strong>{stockCount}</strong> {itemLabel} already exist — stock will be added.{' '}
                            <strong>{newCount}</strong> new will be created.
                          </span>
                        </div>
                      )}
                      <div className="border border-gray-200 rounded-xl overflow-hidden">
                              ...
                              {rows.slice(0, 10).map((row, i) => {
                                ...
                                return (
                                  <tr
                                    ...
                                    className={cn(
                                      'border-b border-gray-50',
                                      errorRow === i ? 'bg-rose-50' : isExisting ? 'bg-blue-50/30' : '',
                                    )}
                                  >
                                    <td
                                      className={cn(
                                        'px-3 py-2',
                                        errorRow === i ? 'text-rose-600 font-bold' : 'text-gray-400',
                                      )}
                                    >
                                      {i + 2}
                                      {errorRow === i && ' ⚠️'}
                      ...
                      <button
                        ...
                        disabled={importing}
                        className="w-full py-3 bg-brand text-white rounded-xl font-bold hover:bg-brand-dark disabled:opacity-60 flex items-center justify-center gap-2"
                      >
                        <Upload size={16} />{' '}
                        {importing
                          ? 'Importing...'
                          : `Import ${rows.length} ${itemLabel}${stockCount > 0 ? ` (${newCount} new, ${stockCount} restock)` : ''}`}
                      </button>
                    </>
                  );
                })()}
