    ...
    >
      ...
      >
        <div className="flex items-start gap-3 mb-4">
          <div className={cn("p-2 rounded-xl shrink-0", variant === 'danger' ? 'bg-rose-100' : variant === 'warning' ? 'bg-amber-100' : 'bg-blue-100')} aria-hidden="true">
          ...
          <div>
            ...
            <p id={descId} className="text-sm text-gray-500 mt-1">{message}</p>
          </div>
        </div>
        <div className="flex gap-3">
          ...
          <button ref={confirmRef} type="button" onClick={onConfirm} className={cn("flex-1 py-2.5 text-white rounded-xl font-bold text-sm", btnCls)}>{confirmLabel}</button>
        </div>
      </motion.div>
    </motion.div>
  );
