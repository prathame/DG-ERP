...
export function InventoryView({
  ...
  const inventoryTrackingEnabled = (() => {
    try {
      const u = session.getUser() || {};
      return u.inventoryTrackingEnabled !== false;
    } catch {
      return true;
    }
  })();
  const warrantyVisible = (() => {
    try {
      const u = session.getUser() || {};
      return u.tabConfig?.warranty?.visible !== false;
    } catch {
      return true;
    }
  })();
  ...
  useEffect(() => {
    api.products
      .list(debouncedBarcodeSearch || undefined)
      .then(setProducts)
      .catch(() => setProducts([]))
            ...
            >
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    {(() => {
                      ...
                      if (!code) return null;
                      if (!/^\d+$/.test(code))
                        return <p className="text-[11px] text-rose-500 mt-1">HSN must be numeric only</p>;
                      if (![4, 6, 8].includes(code.length))
                        return <p className="text-[11px] text-rose-500 mt-1">HSN must be 4, 6, or 8 digits</p>;
                      ...
                      return h ? (
                        ...
                        <p className="text-[11px] text-amber-500 mt-1">
                          Valid format — not in our database, set GST rate manually
                        </p>
                      );
                    })()}
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-400 uppercase">GST Rate (%)</label>
                    <input
                      type="number"
                      min={0}
                      max={40}
                ...
                {addForm.packSize > 1 ? (
                  <>
                    <div>
                      <label className="text-xs font-bold text-gray-400 uppercase">Pieces per Box</label>
                      <input
                        type="text"
                        inputMode="numeric"
                        pattern="[0-9]*"
                        ...
                        onChange={e => {
                          ...
                          setAddForm({ ...addForm, packSize: v === '' ? 0 : parseInt(v, 10) });
                        }}
                        onBlur={() => {
                          if (addForm.packSize < 2) setAddForm({ ...addForm, packSize: 2 });
                        }}
                        className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand"
                ...
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase">Reward Points</label>
                  <input
                    type="number"
                    min={0}
                    value={addForm.rewardPointsValue || ''}
                    onChange={e =>
                      setAddForm({ ...addForm, rewardPointsValue: e.target.value === '' ? 0 : Number(e.target.value) })
                    }
                    className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand"
                  />
                </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer text-xs">
                        <input
                          type="checkbox"
                          checked={addForm.warrantyApplicable}
                          onChange={e => setAddForm({ ...addForm, warrantyApplicable: e.target.checked })}
                          className="rounded text-brand"
                        />
                    ...
                    {addForm.warrantyApplicable && (
                      <input
                        type="number"
                        placeholder="Months"
                        value={addForm.warrantyMonths || ''}
                        onChange={e =>
                          setAddForm({ ...addForm, warrantyMonths: e.target.value === '' ? 0 : Number(e.target.value) })
                        }
                        className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand"
                      />
                    )}
              ...
              >
                {(addStockModal.packSize || 1) > 1 ? (
                  <div>
                    ...
                    <input
                      ...
                      min={1}
                      max={10000}
                      value={addStockForm.packs || ''}
                      onChange={e => setAddStockForm({ ...addStockForm, packs: parseInt(e.target.value) || 0 })}
                      className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand"
                      placeholder="0"
                    />
                    <p className="text-xs text-emerald-600 font-medium mt-1">
                ...
                {addStockModal.barcodeRange && barcodeSystemEnabled && !isQtyStockUnit(addStockModal.packName) && (
                  <p className="text-xs text-gray-500 bg-gray-50 px-3 py-2 rounded-lg">
                    ...
                    continue after <span className="font-mono font-medium">{addStockModal.barcodeRange.last}</span>
                  </p>
                )}
                <div className="flex gap-2 pt-2">
                  <button
            ...
            >
              <div className="p-6 border-b border-gray-100">
                <h3 className="text-lg font-bold flex items-center gap-2">
                  <Barcode size={22} className="text-brand" /> Barcode details — {barcodeDetailsModal.product.name}
                </h3>
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-gray-50 sticky top-0">
                      <tr className="text-xs font-bold text-gray-400 uppercase">
                        ...
                        <th className="px-6 py-3">Barcode range</th>
                        ...
                        <tr key={i}>
                          ...
                          <td className="px-6 py-3">
                            <button
                              type="button"
                              onClick={() => {
                                setLabelPrinterId(barcodeDetailsModal.product.id);
                                setLabelBarcodeRange({ first: b.barcodeFirst, last: b.barcodeLast });
                              }}
                              className="flex items-center gap-1 px-2 py-1 text-xs font-bold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg"
                            >
              ...
              <div className="p-4 border-t border-gray-100">
                <button
                  type="button"
                  onClick={() => setBarcodeDetailsModal(null)}
                  className="w-full py-2 border border-gray-200 rounded-xl font-medium hover:bg-gray-50"
                >
                  Close
                </button>
              </div>
            ...
            >
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-gray-500 mb-8">
                  ...
                  <span className="font-bold text-gray-900">"{productToDelete.name}"</span>? This action cannot be
                  undone.
                </p>
      ...
      {csvImportOpen && (
        <CsvImport
          ...
          columns={[
            ...
            { key: 'costPrice', label: 'Cost (what you paid)' },
            { key: 'barcodePrefix', label: 'Barcode Prefix (Piece/Nos only)' },
            ...
            { key: 'packName', label: 'Unit (Bag/Box/Kg = qty; Piece/Nos = barcodes)' },
          ...
          onImport={async rows => {
            try {
              ...
              const result = await fetchApi<{ success: number; errors: string[] }>('/products/batch', {
                method: 'POST',
              ...
              return {
                success: 0,
                errors: [err instanceof Error ? err.message : 'Import failed — no products were added'],
              };
            }
          }}
        />
