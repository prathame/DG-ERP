...
export function ReplacementsView({ user }: { user: { id: string; role?: string; vendorId?: string } | null }) {
  ...
  const [oldBarcodeValidation, setOldBarcodeValidation] = useState<{
    valid: boolean;
    vendorId?: string;
    vendorName?: string;
  ...
  const handleValidateOld = () => {
    if (!form.oldBarcode.trim()) {
      setOldBarcodeValidation(null);
      return;
    }
    setOldBarcodeValidation(null);
    api.replacements
      .validateOld(form.oldBarcode.trim(), userVendorId ?? undefined)
      .then(r => {
  ...
  const handleValidateNew = () => {
    ...
    if (!vendorIdToCheck) {
      setNewBarcodeValidation({ valid: false, error: 'Verify old barcode first to get vendor' });
      return;
    }
    setNewBarcodeValidation(null);
    api.replacements
      .validateNew(form.newBarcode.trim(), vendorIdToCheck)
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold">{getTabLabel('replacements', 'Replacements')}</h2>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-50 flex items-center gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Search by barcode, product or customer..."
              value={search}
          ...
          <span className="text-sm text-gray-500">
            {filtered.length} replacement{filtered.length !== 1 ? 's' : ''}
          </span>
        </div>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-gray-500">
                    No replacements recorded yet. Record one from Warranty details or add manually.
                  </td>
                </tr>
              ) : (
                filtered.map(r => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-3 sm:px-6 sm:py-4 font-mono text-sm font-bold text-emerald-600">
                      {r.newBarcode}
                    </td>
                    <td className="px-3 py-3 sm:px-6 sm:py-4">
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-sm text-gray-500 mb-4">
                Product under warranty was damaged and replaced with new unit. Verify both barcodes before saving.
              </p>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={e => {
                        setForm({ ...form, oldBarcode: e.target.value });
                        setOldBarcodeValidation(null);
                        setNewBarcodeValidation(null);
                      }}
                      placeholder="Barcode of damaged product"
                      className="flex-1 px-4 py-2 border border-gray-200 rounded-lg font-mono focus:ring-2 focus:ring-brand"
                    />
                    <button
                      type="button"
                    ...
                    >
                      {oldBarcodeValidation.valid ? (
                        <div className="text-emerald-800 space-y-0.5">
                          <p>
                            <strong>Vendor:</strong> {oldBarcodeValidation.vendorName}{' '}
                            {oldBarcodeValidation.productName && <> • {oldBarcodeValidation.productName}</>}
                          </p>
                          {'customerName' in oldBarcodeValidation && oldBarcodeValidation.customerName && (
                            <p>
                              <strong>Customer prefetched:</strong> {oldBarcodeValidation.customerName} •{' '}
                              {oldBarcodeValidation.customerPhone ?? ''}
                            </p>
                          )}
                        </div>
                      ) : (
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={e => {
                        setForm({ ...form, newBarcode: e.target.value });
                        setNewBarcodeValidation(null);
                      }}
                      placeholder="Barcode of new product given"
                      className="flex-1 px-4 py-2 border border-gray-200 rounded-lg font-mono focus:ring-2 focus:ring-brand"
                    />
                    <button
                      type="button"
                ...
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Customer Phone</label>
                  <input
                    required
                ...
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Replaced Date</label>
                  <input
                    type="date"
                    value={form.replacedDate}
                    onChange={e => setForm({ ...form, replacedDate: e.target.value })}
                    className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand"
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Reason (optional)</label>
                  <input
                    value={form.reason}
                    onChange={e => setForm({ ...form, reason: e.target.value })}
                    placeholder="e.g. Damaged, Defect"
                    className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand"
                  />
                </div>
