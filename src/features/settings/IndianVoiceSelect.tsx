...
export function IndianVoiceSelect() {
  ...
  const [assist, setAssist] = useState(getVoiceAssistEnabled);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="font-semibold text-sm">{t('settings.voiceAssist')}</p>
          <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">{t('settings.voiceAssistDesc')}</p>
        </div>
        <button
          type="button"
          role="switch"
          aria-checked={assist}
          aria-label={t('settings.voiceAssist')}
          onClick={() => {
            const next = !assist;
        ...
        >
          <span
            ...
            className={cn(
              ...
              assist ? 'translate-x-5' : 'translate-x-0',
            )}
