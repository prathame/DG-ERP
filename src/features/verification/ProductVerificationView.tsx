              ...
              {searchResults.vendors.map(v => (
                <button
                  ...
                  onClick={() => {
                    api.vendorFinance
                      .detail(v.id)
                      .then(d => setVendorDetail(d as unknown as Record<string, unknown>))
                      .catch(() => {});
                  }}
                  className="w-full px-4 py-3 text-left hover:bg-orange-50 border-b border-gray-50 last:border-0"
                >
          ...
          {(searchResults.staff?.length ?? 0) > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="px-4 py-3 bg-gray-50 border-b border-gray-100 flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-gray-400 uppercase">Staff ({searchResults.staff!.length})</span>
              </div>
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => {
                const d = vendorDetail as {
                  vendor?: { name: string; phone?: string };
                  totalDistributedValue: number;
                  totalPaid: number;
                  balance: number;
                  payments?: { amount: number; paymentDate: string; paymentMethod: string }[];
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div className="bg-blue-50 rounded-xl p-2.5">
                    ...
                    <p className="text-sm sm:text-lg font-bold text-blue-700 truncate">
                      ₹{(d.totalDistributedValue ?? 0).toLocaleString()}
                    </p>
                  </div>
                  <div className="bg-emerald-50 rounded-xl p-2.5">
                    ...
                    <p className="text-sm sm:text-lg font-bold text-emerald-700 truncate">
                      ₹{(d.totalPaid ?? 0).toLocaleString()}
                    </p>
                  </div>
                  <div className={cn('rounded-xl p-2.5', (d.balance ?? 0) > 0 ? 'bg-rose-50' : 'bg-emerald-50')}>
                    <p className="text-[9px] font-bold text-gray-400 uppercase">Balance</p>
                    <p
                      className={cn(
                        'text-sm sm:text-lg font-bold truncate',
                        (d.balance ?? 0) > 0 ? 'text-rose-700' : 'text-emerald-700',
                      )}
                    >
              ...
              <p className="text-lg font-bold text-amber-700">
                ...
                {Math.max(
                  ...
                  staffDetail.payments.reduce(
                    (s, p) =>
                      s + (p.paymentType === 'advance' ? p.amount : p.paymentType === 'advance_repay' ? -p.amount : 0),
                    0,
                  ),
                ).toLocaleString()}
              </p>
