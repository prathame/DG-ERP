...
export function MobileTenantPanel({ tenantId, phone }: Props) {
  ...
  useEffect(() => { void load().catch(() => toast('Failed to load mobile status', 'error')); }, [load, toast]);
  ...
  const waShare = () => {
    ...
    window.open(`https://wa.me/${p ? (p.startsWith('91') ? p : '91' + p) : ''}?text=${encodeURIComponent(text)}`, '_blank');
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="border border-gray-100 rounded-xl p-4 space-y-3">
          ...
          {code ? (
            <>
              <div className="flex items-center justify-between gap-2 bg-gray-50 rounded-xl px-3 py-2.5">
                ...
                <button type="button" onClick={() => { void navigator.clipboard.writeText(code); toast('Copied', 'success'); }} className="p-1.5 hover:bg-gray-200 rounded-lg">
                  <Copy size={14} />
                </button>
              </div>
              {expiresAt && (
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" disabled={busy === 'invite'} onClick={() => void issueInvite()}
            ...
            {code && (
              <button type="button" onClick={waShare}
          ...
          <p className="text-[11px] text-gray-400">Company code (slug): <span className="font-mono text-gray-600">{slug || '—'}</span></p>
        </div>
        ...
        <div className="border border-gray-100 rounded-xl p-4 space-y-3">
          ...
          <button type="button" disabled={busy === 'sync'} onClick={() => void forceSync()}
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            <div>
              ...
              <input value={minVersion} onChange={(e) => setMinVersion(e.target.value)} placeholder="2.1.0"
            ...
            <div>
              ...
              <input value={latestVersion} onChange={(e) => setLatestVersion(e.target.value)} placeholder="2.2.0"
                className="w-full mt-1 px-2 py-1.5 border border-gray-200 rounded-lg text-xs" />
            </div>
          </div>
          <button type="button" disabled={busy === 'ver'} onClick={() => void saveVersions()}
              ...
              <tbody>
                {devices.map((d) => (
                  <tr key={d.id} className="border-t border-gray-50">
                    ...
                    <td className="px-3 py-2">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${d.isOnline ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>
