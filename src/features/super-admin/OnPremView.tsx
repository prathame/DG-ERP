...
function summarizeSettings(s: Record<string, unknown> | undefined | null) {
  ...
  const tabsOn = Object.entries(tc)
    .filter(([, v]) => v?.visible !== false)
    .map(([k, v]) => v?.label || k);
  const tabsOff = Object.entries(tc)
    .filter(([, v]) => v?.visible === false)
    .map(([k, v]) => v?.label || k);
...
export function OnPremView({ saToken }: { saToken: string }) {
  ...
  const [settingsTab, setSettingsTab] = useState<'tabs' | 'features' | 'updates' | 'notify'>('tabs');
  ...
  const [form, setForm] = useState({
    companyName: '',
    businessType: 'manufacturer' as (typeof BUSINESS_TYPES)[number],
    adminEmail: '',
    maxUsers: 5,
    validUntil: '',
  });
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
          ...
          <div className="ml-auto flex items-center gap-2">
            <span
              className={cn(
                ...
                selected.isOnline ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500',
              )}
            >
          ...
          {[
            {
              ...
              color: selected.status === 'active' ? 'text-emerald-600' : 'text-rose-600',
            },
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <div className="flex gap-2 mt-3">
            <a
              href={`https://wa.me/?text=${whatsappMsg(selected)}`}
              target="_blank"
        ...
        {selected.machineId && (
          <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <button
              onClick={() => handleUpdate(selected.id, { clearMachine: true })}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold flex items-center gap-2">
              <Settings size={16} /> Remote Settings
            </h3>
          ...
          {settingsTab === 'features' && (
            <div className="space-y-3">
              <p className="text-xs text-gray-400 mb-3">
                Toggle system features. Changes push to the on-prem app on next sync.
              </p>
              {[
                {
                  key: 'barcodeSystemEnabled',
                  label: 'Barcode System',
                  desc: 'Auto-generated barcodes, scanner, label printing',
                },
                {
                  key: 'multiLanguageEnabled',
                  label: 'Multi-Language',
                  desc: 'Switch UI between English, Hindi, Gujarati',
                },
                {
                  key: 'inventoryTrackingEnabled',
                  label: 'Inventory Tracking',
                  desc: 'Stock count, barcode quantity, Add Stock',
                },
                {
                  key: 'vendorPortalEnabled',
                  label: 'Vendor Portal',
                  desc: 'Vendors get login credentials and portal access',
                },
                {
                  key: 'quotationsEnabled',
                  label: 'Quotations',
                ...
                const val = (localSettings[key] ??
                  (selected?.settings as Record<string, unknown>)?.[key] ??
                  true) as boolean;
                return (
                  <div key={key} className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() =>
                        setLocalSettings(prev => {
                          ...
                          if (key === 'chatbotEnabled') {
                            const baseTc = {
                              ...((selected?.settings?.tabConfig as Record<
                                string,
                      ...
                      className={cn(
                        'relative inline-flex h-5 w-9 rounded-full border-2 border-transparent transition-colors shrink-0 ml-4',
          ...
          {settingsTab === 'notify' && (
            <div className="space-y-3">
              <p className="text-xs text-gray-400">
                Queues a Bell message on cloud. The on-prem app pulls it on the next heartbeat or hard sync (Sync Now /
                Push to device).
              </p>
              <input
              ...
              <textarea
                ...
                placeholder="Message"
                rows={3}
                className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-brand resize-none"
              />
              <select
              ...
              <button
                type="button"
                disabled={sendingNotify || !notifyForm.title.trim() || !notifyForm.message.trim()}
                onClick={async () => {
                  if (!selected) return;
                  setSendingNotify(true);
                  try {
                    const res = await fetch(`/api/super-admin/onprem/${selected.id}/notify`, {
                      method: 'POST',
                      headers: h,
                    ...
                    if (!res.ok) {
                      ...
                      toast((err as { error?: string }).error || 'Failed to queue notification', 'error');
                    } else {
                      toast('Queued — device picks it up on next sync', 'success');
                      setNotifyForm({ title: '', message: '', type: 'info' });
                    }
                  } catch {
                    toast('Failed to queue notification', 'error');
                  } finally {
                    setSendingNotify(false);
                  }
                }}
                className="w-full py-2.5 bg-brand text-white rounded-xl text-sm font-bold hover:bg-orange-600 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                <Bell size={14} />
                {sendingNotify ? 'Queuing…' : 'Queue Bell message'}
              </button>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div
                className={cn(
                  ...
                  selected?.appVersion ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200',
                )}
              >
              ...
              <p className="text-xs text-gray-400">
                Control which version this installation runs. Applied on next heartbeat (up to 15 min).
              </p>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs font-bold text-gray-400 uppercase">
                    Latest Version (shows update notification)
                  </label>
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (r.ok) {
                          const releases = (await r.json()) as {
                            tag_name: string;
                            name: string;
                            published_at: string;
                          }[];
                  ...
                  >
                    {releasesLoading
                      ...
                      : githubReleases.length
                        ? `${githubReleases.length} releases loaded`
                        : 'Load from GitHub'}
                  </button>
                </div>
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase block mb-1">
                  Minimum Version (force update — blocks old)
                </label>
                {githubReleases.length > 0 ? (
                  <select
                    value={minVersion}
              ...
              <div className="mt-5 space-y-3">
                {onLocalhost && (
                  <div className="rounded-xl bg-red-50 text-red-800 px-3 py-2.5 text-xs leading-relaxed">
                    ...
                    <p>
                      This Super Admin is <b>localhost</b>. The on-prem app syncs to <b>dhandho-2kdx.onrender.com</b>.
                      Device can apply settings, but this page never sees “applied”.
                    </p>
                    <p className="mt-1.5 font-medium">Pick one:</p>
                    <ul className="list-disc ml-4 mt-1 space-y-0.5">
                      <li>
                        Open Super Admin on Render:{' '}
                        <span className="font-mono">https://dhandho-2kdx.onrender.com/admin</span>
                      </li>
                      <li>
                ...
                >
                  ...
                  {pushing
                    ? pushPhase === 'waiting'
                      ? 'Waiting for device confirm…'
                      : 'Sending…'
                    : dirty
                      ? 'Push to device'
                      : 'Push to device again'}
                </button>

                <div className="text-[10px] font-bold uppercase tracking-wide text-gray-400 px-0.5">
                  Status (not a button)
                </div>
                <div
                  className={cn(
                    'rounded-xl px-4 py-3 text-sm pointer-events-none select-none',
                    pushPhase === 'done' || (selected.settingsPushedAt && !pending)
                ...
                >
                  ...
                  {pushPhase === 'idle' && !selected.settingsPushedAt && <p className="font-bold">Not sent yet</p>}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-3">
          ...
          {selected.status === 'active' ? (
            <button
              onClick={() => handleUpdate(selected.id, { status: 'suspended' })}
            ...
            <button
              onClick={() => handleUpdate(selected.id, { status: 'active' })}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold flex items-center gap-2">
            <Monitor size={20} /> On-Prem Installations
          </h2>
              ...
              {licenses.map(lic => (
                <tr key={lic.id} className="hover:bg-gray-50 cursor-pointer" onClick={() => setSelected(lic)}>
                  ...
                  <td className="px-5 py-3 text-gray-500">
                    {lic.validUntil ? new Date(lic.validUntil).toLocaleDateString('en-IN') : 'Lifetime'}
                  </td>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Shah Seeds Pvt Ltd"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={e =>
                    setForm(f => ({ ...f, businessType: e.target.value as (typeof BUSINESS_TYPES)[number] }))
                  }
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand text-sm"
                  placeholder="admin@shahseeds.com (optional)"
                />
              </div>
              ...
              <div className="flex gap-2 pt-2">
                <button onClick={() => setShowNew(false)} className="flex-1 py-2 border rounded-lg font-medium text-sm">
                  Cancel
                </button>
                <button onClick={handleCreate} className="flex-1 py-2 bg-brand text-white rounded-lg font-bold text-sm">
                  Generate License
                </button>
              </div>
            </div>
