/**
 * Unified Tenants tab — Cloud + On-Prem + Service Mobile with toggle at top.
 */
...
export function TenantsView({ onSelectTenant }: TenantsViewProps) {
  ...
  return (
    <div className="space-y-5">
      {/* Toggle */}
      <div className="flex items-center gap-2 bg-gray-100 p-1 rounded-xl w-fit flex-wrap">
        <button
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all',
            mode === 'service-mobile' ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700',
          )}
        >
          <Smartphone size={15} /> Service Mobile
        </button>
      </div>

      {mode === 'cloud' && <TenantListView onSelectTenant={onSelectTenant} />}
      {mode === 'onprem' && <OnPremView saToken={session.getToken() || ''} />}
      {mode === 'service-mobile' && <ServiceMobileView saToken={session.getToken() || ''} />}
    </div>
  );
}
