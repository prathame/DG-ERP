...
function Step({ n, title, children }: { n: number; title: string; children: React.ReactNode }) {
  return (
    <div className="flex gap-4">
      <div className="w-7 h-7 rounded-full bg-brand text-white text-sm font-bold flex items-center justify-center shrink-0 mt-0.5">
        {n}
      </div>
    ...
    <div className="space-y-2">
      {items.map((item, i) => (
        ...
        >
          ...
          <span className={cn('text-sm', checked.includes(i) ? 'line-through text-gray-400' : 'text-gray-700')}>
            {item}
          </span>
        </div>
      ))}
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 max-w-4xl">
      <div>
        <h2 className="text-xl font-bold flex items-center gap-2">
          <BookOpen size={22} /> Developer Guide
        </h2>
        <p className="text-sm text-gray-500 mt-1">
          Setup, testing, and deployment instructions for all three Dhandho delivery modes
        </p>
      </div>
      ...
      <Section title="Onboard a Cloud Customer" icon={Rocket} color="bg-brand" defaultOpen>
        <p className="text-sm text-gray-600">
          Customer wants cloud access — pays monthly, you host everything, works from any browser or the Cloud Electron
          app.
        </p>
        ...
        <div className="space-y-4">
          <Step n={1} title="Create the tenant">
            <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5 text-gray-700">
              ...
              <p>
                Click <strong>Create Tenant</strong>
              </p>
            </div>
          </Step>
          ...
          <Step n={2} title="Share credentials with customer">
            <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5 text-gray-700">
              ...
              <p>
                • Login URL: <code className="bg-gray-200 px-1 rounded">{window.location.origin}/their-slug</code>
              </p>
              <p>• Admin email + temporary password</p>
              <p>
                Click <strong>Share via WhatsApp</strong> — includes web login credentials
              </p>
            </div>
          </Step>
          ...
          <Step n={3} title="Customer logs in">
            <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5 text-gray-700">
              <p>
                <strong>Web:</strong> opens the link → logs in → change password
              </p>
              <p>
                <strong>Desktop:</strong> download Cloud Electron from{' '}
                <code className="bg-gray-200 px-1 rounded">/download</code>
              </p>
            </div>
          </Step>
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
            <p className="text-sm text-emerald-800 font-medium">
              ✅ Cloud customer is live. Updates are instant — just deploy code changes.
            </p>
          </div>
        </div>
      </Section>
      ...
      <Section title="Onboard an On-Prem Customer" icon={Users} color="bg-purple-500" defaultOpen>
        <p className="text-sm text-gray-600">
          Customer wants software on their own PC — pays one-time license fee, data stays with them, works offline.
        </p>
        ...
        <div className="space-y-4">
          <Step n={1} title="Issue a license key">
            <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5 text-gray-700">
              <p>
                Super Admin → <strong>On-Prem</strong> → Issue License
              </p>
              <p>Fill:</p>
              <p>
              ...
              <p>
                • <strong>Max users</strong> — 1, 3, 5, 10, unlimited
              </p>
              <p>
                • <strong>Valid until</strong> — license expiry date
              </p>
              <p>
                Click <strong>Generate License</strong>
              </p>
            </div>
          </Step>
          ...
          <Step n={2} title="Send installer + license key to customer">
            <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5 text-gray-700">
              <p>
                After generating, click <strong>Share via WhatsApp</strong>
              </p>
              <p>Customer receives:</p>
              <p>• Download link for installer (.exe or .dmg)</p>
              <p>
                • Their license key: <code className="bg-gray-200 px-1 rounded font-mono">DG-XXXX-XXXX-XXXX</code>
              </p>
            </div>
            <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 text-xs text-amber-700">
              ⚠️ Windows may show a security warning on install (Unknown Publisher). Tell customer: click "More info" →
              "Run anyway". This is normal until you buy a code signing certificate.
            </div>
          </Step>
          ...
          <Step n={3} title="Customer installs and activates">
            <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5 text-gray-700">
              ...
              <p>
                3. Enters license key → <strong>company name loads automatically</strong>
              </p>
          ...
          <Step n={4} title="Verify in super admin">
            <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5 text-gray-700">
              <p>
                On-Prem tab → their installation shows as <strong>● Online</strong>
              </p>
            ...
            <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5 text-gray-700">
              ...
              <p>
                <code className="bg-gray-200 px-1.5 py-0.5 rounded font-mono">
                  http://192.168.1.X:3001/company-slug
                </code>
              </p>
      ...
      <Section title="On-Prem Electron App — Testing" icon={Monitor} color="bg-purple-500">
        ...
        <Step n={1} title="Create a test license in super admin">
          <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5 font-mono text-gray-700">
            ...
            <p>
              5. <strong>Copy the key</strong> — DG-XXXX-XXXX-XXXX
            </p>
          </div>
        </Step>
      ...
      <Section title="Online Status Indicator — Testing" icon={Terminal} color="bg-emerald-500">
        <p className="text-sm text-gray-600">
          The sidebar indicator shows connection status in on-prem mode. Test it in the browser without Electron.
        </p>

        <Step n={1} title="Enable on-prem mode in frontend">
          <p className="text-xs text-gray-500 mb-2">
            Add to <code className="bg-gray-100 px-1.5 py-0.5 rounded">.env</code> in project root:
          </p>
        ...
        <Step n={2} title="Restart dev server">
          ...
          <p className="text-xs text-gray-500">
            Bottom of sidebar shows <strong>● Online · Synced</strong> or <strong>⚪ Offline</strong>
          </p>
        </Step>

        <Step n={3} title="Remove after testing">
          <p className="text-xs text-gray-500">
            Delete or comment out <code className="bg-gray-100 px-1.5 py-0.5 rounded">VITE_DEPLOYMENT_MODE=onprem</code>{' '}
            from <code className="bg-gray-100 px-1.5 py-0.5 rounded">.env</code> — cloud users must never see this
            indicator.
          </p>
        </Step>
      </Section>
      ...
      <Section title="License Management — How-To" icon={Monitor} color="bg-orange-500">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-600">
              On-Prem tab → Issue License → fill details → Generate. Share key + download link via WhatsApp button.
            </p>
          </div>
          <div></div>
          <div>
            ...
            <p className="text-sm text-gray-600">
              Click the license row → Manage → Suspend (temporary) or Revoke (permanent). App shows blocked message on
              next heartbeat (up to 60 min).
            </p>
          </div>
          <div>
            <h4 className="font-bold text-sm mb-2">Transfer to new PC</h4>
            <p className="text-sm text-gray-600">
              Click license row → Transfer License (clears machine binding). Customer runs app on new PC → enters same
              key → activates.
            </p>
          </div>
          <div>
            ...
            <p className="text-sm text-gray-600">
              ● Online = heartbeat received in last 70 minutes. ⚪ Offline = no heartbeat. App works fine offline —
              status just reflects connectivity.
            </p>
          </div>
        </div>
      </Section>
      ...
      <Section title="Testing Checklist" icon={CheckSquare} color="bg-gray-600" defaultOpen>
        <p className="text-sm text-gray-600 mb-2">
          Click each item to mark as done. Verify before shipping any release.
        </p>
