                ...
                <tbody className="divide-y divide-gray-100">
                  {payments.map(p => (
                    <tr key={p.id} className="hover:bg-gray-50">
                      ...
                      {canEdit && <td className="px-4 py-3"><button type="button" onClick={async () => { if (!confirm('Delete this payment?')) return; try { await api.payroll.delete(p.id); toast('Deleted', 'success'); load(); } catch(e) { toast((e as Error).message, 'error'); } }} className="p-1 text-rose-400 hover:text-rose-600"><Trash2 size={14} /></button></td>}
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative bg-white w-full max-w-md rounded-2xl shadow-xl p-6">
              ...
              <div className="space-y-3">
                ...
                <div><label className="text-xs font-bold text-gray-400 block mb-1">Reference / UTR</label><input value={form.referenceNumber} onChange={e => setForm({ ...form, referenceNumber: e.target.value })} className="w-full px-4 py-2 border border-gray-200 rounded-lg" placeholder="Optional" /></div>
