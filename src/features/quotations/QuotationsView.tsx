...
export function QuotationsView() {
  ...
  const handleCreate = async () => {
    ...
    try {
      await fetchApi('/quotations', {
        method: 'POST',
        body: JSON.stringify({
          vendorId: form.vendorId || undefined, customerName: form.customerName || undefined,
          customerPhone: form.customerPhone || undefined, customerEmail: form.customerEmail || undefined,
          quotationDate: form.date, validUntil: form.validUntil || undefined, gstRate: defaultGstRate,
          notes: form.notes || undefined,
          items: validRows.map(r => ({ productId: r.productId, quantity: r.quantity, customPrice: r.customPrice ? parseFloat(r.customPrice) : undefined, discountPercent: r.discount > 0 ? r.discount : undefined, withGst: r.withGst })),
        }),
      });
      setModalOpen(false); setRows([{ productId: '', quantity: 1, customPrice: '', discount: 0, withGst: true }]);
      setForm({ vendorId: '', customerName: '', customerPhone: '', customerEmail: '', date: new Date().toISOString().slice(0, 10), validUntil: '', notes: '' });
      load(); toast('Quotation created', 'success');
  ...
  const handleConvert = async (q: Quotation) => {
    ...
    try {
      ...
      load(); setSelectedId(null); setSelected(null);
    } catch (err) { toast((err as Error).message, 'error'); }
  };
  ...
  const handleStatusChange = async (q: Quotation, status: string) => {
    try {
      ...
      load(); if (selected) setSelected({ ...selected, status });
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 bg-gray-50 border-b border-gray-100 flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3 flex-wrap">
              <button type="button" onClick={() => { setSelectedId(null); setSelected(null); }} className="p-2 hover:bg-gray-200 rounded-lg"><ArrowLeft size={20} className="text-gray-600" /></button>
              <h3 className="font-bold text-lg">{selected.quotationNumber}</h3>
              <span className={cn("px-2.5 py-1 rounded-full text-xs font-bold", statusColors[selected.status] || 'bg-gray-100')}>{selected.status}</span>
              {selected.status === 'Draft' && <button type="button" onClick={() => handleStatusChange(selected, 'Sent')} className="flex items-center gap-1 px-3 py-1.5 text-sm font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg"><Send size={14} /> Mark Sent</button>}
              {selected.status === 'Sent' && <button type="button" onClick={() => handleStatusChange(selected, 'Accepted')} className="flex items-center gap-1 px-3 py-1.5 text-sm font-bold text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-lg"><Check size={14} /> Accepted</button>}
              {(selected.status === 'Accepted' || selected.status === 'Draft') && selected.vendorId && (
                <button type="button" onClick={() => handleConvert(selected)} className="flex items-center gap-1 px-3 py-1.5 text-sm font-bold text-purple-600 bg-purple-50 hover:bg-purple-100 rounded-lg"><ArrowRight size={14} /> Convert to Distribution</button>
              )}
              {selected.customerPhone && (
                <button type="button" onClick={() => shareViaWhatsApp(selected.customerPhone!, formatQuotationText(selected))} className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-green-600 hover:bg-green-50 rounded-lg"><MessageCircle size={14} /> WhatsApp</button>
              )}
              {(selected.status === 'Draft' || selected.status === 'Rejected') && (
                <button type="button" onClick={() => handleDelete(selected)} className="flex items-center gap-1 px-3 py-1.5 text-sm font-medium text-rose-600 hover:bg-rose-50 rounded-lg"><Trash2 size={14} /> Delete</button>
              )}
            </div>
            <div className="text-right">
              <span className="text-sm text-gray-600">Date: {formatDate(selected.quotationDate)}</span>
              {selected.validUntil && <span className="text-sm text-gray-500 ml-2">Valid until: {formatDate(selected.validUntil)}</span>}
            </div>
          </div>
          <div className="p-6">
            {selected.customerName && <p className="text-sm text-gray-600 mb-1">To: <strong>{selected.customerName}</strong> {selected.customerPhone && `• ${selected.customerPhone}`}</p>}
            ...
            <button key={q.id} type="button" onClick={() => { setSelectedId(q.id); fetchApi<Quotation>(`/quotations/${q.id}`).then(setSelected).catch(err => toast(err.message, 'error')); }}
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={cn("px-2 py-0.5 rounded-full text-xs font-bold", statusColors[q.status])}>{q.status}</span>
                </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative bg-white w-full max-w-2xl rounded-2xl shadow-xl max-h-[90vh] overflow-y-auto p-6">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div><label className="text-xs font-bold text-gray-400 uppercase">Vendor / Customer</label><select value={form.vendorId} onChange={e => { const v = vendors.find(x => x.id === e.target.value); setForm({ ...form, vendorId: e.target.value, customerName: v?.name || form.customerName, customerPhone: v?.phone || form.customerPhone }); }} className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand"><option value="">Select or type below</option>{vendors.filter(v => v.id !== 'OWNER').map(v => <option key={v.id} value={v.id}>{v.name}</option>)}</select></div>
                <div><label className="text-xs font-bold text-gray-400 uppercase">Customer Name</label><input value={form.customerName} onChange={e => setForm({ ...form, customerName: e.target.value })} className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand" placeholder="If not a vendor" /></div>
                <div><label className="text-xs font-bold text-gray-400 uppercase">Phone</label><input value={form.customerPhone} onChange={e => setForm({ ...form, customerPhone: e.target.value })} className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand" /></div>
                <div><label className="text-xs font-bold text-gray-400 uppercase">Date</label><input type="date" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand" /></div>
                <div><label className="text-xs font-bold text-gray-400 uppercase">Valid Until</label><input type="date" value={form.validUntil} onChange={e => setForm({ ...form, validUntil: e.target.value })} className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-brand" /></div>
              </div>
              <div className="border border-gray-200 rounded-xl overflow-hidden overflow-x-auto mb-4">
                <table className="w-full text-left"><thead className="bg-gray-50"><tr className="text-xs font-bold text-gray-400 uppercase">
                  <th className="px-3 py-3 w-8">#</th><th className="px-3 py-3">Product</th><th className="px-3 py-3 w-16">Qty</th><th className="px-3 py-3 w-24">Price</th><th className="px-3 py-3 w-16">Disc%</th><th className="px-3 py-3 w-12 text-center">GST</th><th className="px-3 py-3 w-24 text-right">Total</th><th className="px-3 py-3 w-8"></th>
                </tr></thead><tbody className="divide-y divide-gray-100">
                  {rows.map((row, idx) => {
                    const p = products.find(x => x.id === row.productId);
                    const price = row.customPrice ? parseFloat(row.customPrice) : (p?.price ?? 0);
                    const gross = price * (row.quantity || 0);
                    const disc = Math.round(gross * (row.discount || 0) / 100);
                    const net = gross - disc;
                    const gst = row.withGst ? Math.round(net * defaultGstRate / 100) : 0;
                    return (
                      <tr key={idx}>
                        <td className="px-3 py-2 text-xs text-gray-400">{idx + 1}</td>
                        <td className="px-3 py-2"><select value={row.productId} onChange={e => setRows(rows.map((r, i) => i === idx ? { ...r, productId: e.target.value } : r))} className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-sm"><option value="">Select</option>{products.map(pr => <option key={pr.id} value={pr.id}>{pr.name} (₹{pr.price.toLocaleString()})</option>)}</select></td>
                        <td className="px-3 py-2"><input type="number" min={1} value={row.quantity || ''} onChange={e => setRows(rows.map((r, i) => i === idx ? { ...r, quantity: parseInt(e.target.value) || 0 } : r))} className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-sm text-center" /></td>
                        <td className="px-3 py-2"><input type="number" value={row.customPrice} onChange={e => setRows(rows.map((r, i) => i === idx ? { ...r, customPrice: e.target.value } : r))} placeholder={p ? `₹${p.price}` : '—'} className={cn("w-full px-2 py-1.5 border rounded-lg text-sm text-center", row.customPrice ? "border-amber-300 bg-amber-50" : "border-gray-200")} /></td>
                        <td className="px-3 py-2"><input type="number" min={0} max={100} value={row.discount || ''} onChange={e => setRows(rows.map((r, i) => i === idx ? { ...r, discount: parseFloat(e.target.value) || 0 } : r))} className="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-sm text-center" /></td>
                        <td className="px-3 py-2 text-center"><input type="checkbox" checked={row.withGst} onChange={e => setRows(rows.map((r, i) => i === idx ? { ...r, withGst: e.target.checked } : r))} className="rounded" /></td>
                        <td className="px-3 py-2 text-right text-sm font-bold">{net + gst > 0 ? `₹${(net + gst).toLocaleString()}` : '—'}</td>
