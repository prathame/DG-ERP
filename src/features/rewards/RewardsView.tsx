...
export function RewardsView({ user }: { user?: { role?: string; vendorId?: string } | null }) {
  ...
  const [rewardsSummary, setRewardsSummary] = useState<{
    vendorSummaries: { vendorName: string; productsSold: number; totalRewardPoints: number }[];
  } | null>(null);
  ...
  useEffect(() => {
    if (vendorId) {
      setLoading(true);
      Promise.all([api.rewards.list(filter !== 'All' ? filter : undefined, vendorId), api.dashboard.vendor(vendorId)])
        .then(([r, d]) => {
          setRewards(r);
            ...
            vendorSummaries: [
              {
                ...
                productsSold: d.vendor.totalSales ?? 0,
                totalRewardPoints: d.vendor.totalRewardPoints ?? 0,
              },
            ],
          });
        })
  ...
  useEffect(() => {
    api.redemptionSettings
      .get()
      .then(setRedemptionSettings)
      .catch(() => setRedemptionSettings({ minBalance: 100, minPoints: 50 }));
  }, []);
  ...
  const handleRedeem = async (e: React.FormEvent) => {
    ...
    if (balance < minBalance) {
      toast(`Minimum balance of ${minBalance} pts required to redeem`, 'error');
      return;
    }
    if (pts < minPoints) {
      toast(`Minimum ${minPoints} pts per redemption`, 'error');
      return;
    }
    ...
    try {
      await api.rewards.create({
        ...
        points: pts,
        type: 'Redeemed',
        description: redeemForm.description || 'Points redeemed',
        vendorId,
      });
          ...
          <div className="bg-[#151619] text-white p-5 sm:p-8 rounded-3xl relative overflow-hidden shadow-2xl">
            ...
            <div className="relative z-10">
              ...
              <h2 className="text-4xl font-bold mb-2">
                {balance.toLocaleString('en-IN')} <span className="text-lg font-normal text-gray-500">pts</span>
              </h2>
              {canRedeem && redemptionSettings && (
                <p className="text-xs text-gray-500 mb-6">
                  Redeem when balance ≥ {minBalance} pts (min {minPoints} pts per redemption)
                </p>
              )}
              ...
              <div className="flex gap-4">
                <button
                  type="button"
                  onClick={() => canRedeem && setRedeemModalOpen(true)}
                  disabled={!canRedeem}
                  className={cn(
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-sm text-gray-500 mb-4">
                Min balance: {minBalance} pts • Min per redemption: {minPoints} pts
              </p>
              <form onSubmit={handleRedeem} className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Points to redeem</label>
                  <input
                    type="number"
                    min={minPoints}
                    max={balance}
                    ...
                    placeholder={`Min ${minPoints} - Max ${balance} pts`}
                  />
                </div>
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Description (optional)</label>
                  <input
