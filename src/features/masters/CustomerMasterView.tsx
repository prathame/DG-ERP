...
export function CustomerMasterView({ onBack, onRefresh, user }: { onBack: () => void; onRefresh: () => void; user?: { role?: string; vendorId?: string } | null }) {
  ...
  const [purchasesModal, setPurchasesModal] = useState<{ customer: Customer; purchases: { productName: string; vendorName: string; barcode: string; purchaseDate: string }[] } | null>(null);
  ...
  const openPurchases = (c: Customer) => {
    api.customers.purchases(c.id).then((purchases) => {
      setPurchasesModal({ customer: c, purchases });
    }).catch(() => setPurchasesModal({ customer: c, purchases: [] }));
  };

  const load = () => {
    api.customers.list(debouncedSearch || undefined, vendorId).then(setList).catch(() => setList([])).finally(() => setLoading(false));
    onRefresh();
  };
  useEffect(() => { setLoading(true); load(); }, [debouncedSearch, vendorId]);

  const openAdd = () => { setEditing(null); setForm({ name: '', phone: '', email: '', address: '', vendorId: vendorId ?? '' }); setModalOpen(true); };
  const openEdit = (c: Customer) => { setEditing(c); setForm({ name: c.name, phone: c.phone ?? '', email: c.email ?? '', address: c.address ?? '', vendorId: c.vendorId ?? '' }); setModalOpen(true); };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitting(true);
    const { vendorId, ...rest } = form;
    (editing ? api.customers.update(editing.id, { ...rest, vendorId: vendorId || null }) : api.customers.create({ ...rest, vendorId: vendorId || null }))
  ...
  const handleDelete = () => {
    ...
    api.customers.delete(deleteTarget.id).then(() => { setDeleteTarget(null); load(); }).catch((err) => toast(err.message, 'error'));
  };
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-50 flex items-center gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input type="text" placeholder="Search customers..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-brand" />
          </div>
        </div>
        ...
        {purchasesModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative bg-white w-full max-w-2xl rounded-2xl shadow-xl overflow-hidden">
              ...
              <div className="p-4 border-t border-gray-100"><button type="button" onClick={() => setPurchasesModal(null)} className="w-full py-2 border border-gray-200 rounded-lg font-medium hover:bg-gray-50">Close</button></div>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
