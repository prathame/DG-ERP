      ...
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">
            See which vendors have which customers. Customers can also purchase directly from factory.
          </p>
        </div>
        <button
          type="button"
          onClick={() => {
            ...
            data.directCustomers.forEach(c =>
              rows.push({
                vendorName: 'Direct',
                vendorPhone: '',
                customerName: c.name,
            ...
            if (rows.length) exportToCsv(rows, 'vendor-customer-mapping');
          }}
          disabled={!data.vendors.length && !data.directCustomers.length}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-6">
          <h3 className="font-bold text-lg flex items-center gap-2">
            <ShoppingCart size={20} className="text-purple-600" /> Vendors & Their Customers
          </h3>
          {data.vendors.map(v => (
            <div key={v.vendor.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="p-4 bg-purple-50 border-b border-purple-100">
                ...
                <p className="text-xs text-purple-600">
                  {v.vendor.contactPerson} • {v.vendor.phone}
                </p>
                <p className="text-xs font-bold text-purple-700 mt-1">
                  {v.customers.length} customer{v.customers.length !== 1 ? 's' : ''}
                </p>
              </div>
              <div className="p-4 space-y-2 max-h-48 overflow-y-auto">
                {v.customers.length === 0 ? (
                  <p className="text-sm text-gray-400 italic">No customers mapped</p>
                ) : (
                  v.customers.map(c => (
        ...
        <div className="space-y-6">
          <h3 className="font-bold text-lg flex items-center gap-2">
            <Package size={20} className="text-orange-600" /> Direct Customers (Factory Purchase)
          </h3>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-4 bg-amber-50 border-b border-amber-100">
              ...
              <p className="text-xs font-bold text-amber-700 mt-1">
                {data.directCustomers.length} customer{data.directCustomers.length !== 1 ? 's' : ''}
              </p>
            </div>
            <div className="p-4 space-y-2 max-h-96 overflow-y-auto">
              {data.directCustomers.length === 0 ? (
                <p className="text-sm text-gray-400 italic">No direct customers</p>
              ) : (
                data.directCustomers.map(c => (
                  <div key={c.id} className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
                    <div>
                      <p className="font-medium text-sm">{c.name}</p>
                      <p className="text-xs text-gray-500">{c.phone || c.email || '-'}</p>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-2 max-h-60 overflow-y-auto">
                <button
                  type="button"
                  onClick={() => handleAssignVendor(assignModal.customer.id, null)}
                  className={cn(
                    'w-full text-left px-4 py-3 rounded-xl border-2 transition-all',
                    assignModal.currentVendorId === null
                ...
                {vendors.map(v => (
                  <button
                    key={v.id}
                    type="button"
                    onClick={() => handleAssignVendor(assignModal.customer.id, v.id)}
                    className={cn(
                      'w-full text-left px-4 py-3 rounded-xl border-2 transition-all',
                      assignModal.currentVendorId === v.id
                        ? 'border-brand bg-orange-50'
                        : 'border-gray-200 hover:border-gray-300',
                    )}
                  >
              ...
              <button
                ...
                className="w-full mt-4 py-2 border border-gray-200 rounded-lg font-medium"
              >
                Cancel
              </button>
            </motion.div>
