...
export function RewardRulesView({ onBack, accessLevel = 'full' }: { onBack: () => void; accessLevel?: AccessLevel }) {
  ...
  const load = () => {
    Promise.all([api.rewardRules.list(), api.redemptionSettings.get()])
      .then(([r, rs]) => {
        setList(r);
        setRedemptionSettings(rs);
        setRedemptionForm({ minBalance: rs.minBalance, minPoints: rs.minPoints });
      })
      .catch(() => {})
  ...
  const openAdd = () => {
    if (!canWrite) return;
    setEditing(null);
    setForm({ productsSoldThreshold: 10, rewardPoints: 100, description: '' });
    setModalOpen(true);
  };
  const openEdit = (r: RewardRule) => {
    ...
    setEditing(r);
    setForm({
      productsSoldThreshold: r.productsSoldThreshold,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    const data = {
      productsSoldThreshold: form.productsSoldThreshold,
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div className="flex-1">
          <h2 className="text-xl font-bold">Reward Rules</h2>
          <p className="text-sm text-gray-500">Define reward milestones for vendor sales</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <p className="text-sm text-gray-500 mb-4">
          When can users redeem points? Set minimum balance and minimum points per redemption.
        </p>
        <form onSubmit={handleRedemptionSave} className="flex flex-wrap items-end gap-4">
          <div>
            <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Min balance to redeem (pts)</label>
            <input
              type="number"
              min={0}
              value={redemptionForm.minBalance || ''}
              onChange={e =>
                setRedemptionForm({ ...redemptionForm, minBalance: e.target.value === '' ? 0 : Number(e.target.value) })
              }
              className="w-32 px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand"
            />
          </div>
          <div>
            <label className="text-xs font-bold text-gray-400 uppercase block mb-1">Min points per redemption</label>
            <input
              type="number"
              min={1}
              value={redemptionForm.minPoints || ''}
              onChange={e =>
                setRedemptionForm({ ...redemptionForm, minPoints: e.target.value === '' ? 0 : Number(e.target.value) })
              }
              className="w-32 px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand"
            />
          </div>
          {canWrite && (
            <button
              type="submit"
              disabled={redemptionSubmitting}
              className="px-4 py-2 bg-brand text-white rounded-xl font-bold text-sm"
            >
              {redemptionSubmitting ? 'Saving...' : 'Save'}
            </button>
          )}
        </form>
      </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase">Products sold threshold</label>
                  <input
                    type="number"
                    min={1}
                    value={form.productsSoldThreshold || ''}
                    onChange={e =>
                      setForm({ ...form, productsSoldThreshold: e.target.value === '' ? 0 : Number(e.target.value) })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setForm({ ...form, rewardPoints: e.target.value === '' ? 0 : Number(e.target.value) })
                    }
                    className="w-full mt-1 px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand"
                  />
                </div>
                <div>
                  ...
                  <input
                    placeholder="e.g. 10 Submersible sold = 100 pts"
