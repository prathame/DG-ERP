...
import { loadLicense, saveLicense, getMachineId } from './license-store';
...
async function startExpressServer(dbUrl) {
...
async function sendHeartbeat() {
    ...
    try {
        ...
        if (!data.licenseValid) {
            ...
            mainWin?.webContents.executeJavaScript(`alert('Your DG ERP license has been ${data.licenseStatus}. Please contact support.')`);
        }
        if (data.forceUpdate) {
            mainWin?.webContents.executeJavaScript(`confirm('A required update is available. Click OK to download.') && window.open('https://dg-erp.in/download')`);
        }
        else if (data.updateAvailable) {
            mainWin?.webContents.executeJavaScript(`console.log('UPDATE_AVAILABLE:${data.latestVersion}')`);
        }
...
ipcMain.handle('complete-setup', async (_event, data) => {
...
// ── Main app window ───────────────────────────────────────────────────────────
async function openMainWindow(slug) {
